"""Generate gga_training_dfs_subsets.ipynb.
...
Step-7 plan (mixed-pool refactor 2026-05-07):
  - Build the Dick 2021 SI II training pool as 26 mixed TrainingPoints
    (21 AE + 3 BH76 + 2 IP13) via xcquinox.pipeline.training_points.
  ...
  - Post-process: the subset-sweep figures and headline.json, the probe figures and tables where the probe evaluations exist
...
NOTEBOOK_OUT = Path(__file__).resolve().parent / "gga_training_dfs_subsets.ipynb"
...
def build_cells(mode: str = "train") -> list:
    ...
    if mode == "subset":
        cells.append(_md(
            ...
            "step-7 training notebook (`gga_training_dfs_subsets.ipynb`) and the\n"
...
def _config_cells() -> list:
    ...
    cells.append(_code(
        ...
        "from xcquinox.pipeline import dfs_pool\n"
...
def _oep_ccsd_cells() -> list:
    ...
    cells.append(_md(
        ...
        "Species in `xcquinox.pipeline.external_refs._PER_SPECIES_OEP_OVERRIDES`\n"
        ...
        "Wu-Yang inversion: Heaton-Burgess, Bulat and Yang [oep-tdl-1];\n"
        "Bulat, Heaton-Burgess, Cohen and Yang [oep-tdl-2]; Bulat and Yang\n"
        "[oep-tdl-4]; Ryabinkin, Kohut and Staroverov [oep-tdl-5]; Kohut,\n"
        "Ryabinkin and Staroverov [oep-tdl-6]. Forward-looking reference\n"
        "[oep-tdl-3] (Hsu and Yang,\n"
        ...
        "sweep fails to converge a species. The tune records of the run behind\n"
        "the overrides are kept under `data/oep_tune/2026-05-06/`. **All citation\n"
        "tags resolve to TO-DOWNLOAD entries at the end of `docs/references.bib`\n"
        "and are AUTHOR-RECALLED, UNVERIFIED** -- verify each against the\n"
        "publisher record before paper write-up; do not promote to `@article`\n"
...
def _smoke_and_training_cells() -> list:
    ...
    cells.append(_md(
        ...
        "and call `pipeline.run_test`. Produces `aggregate.json` + `per_molecule.json`\n"
        "under `spec_dir/eval/`.\n\n"
    ...
    cells.append(_code(
        ...
        "# ('ae_kcalmol' field) -- see xcquinox/pipeline/dfs_pool.py for\n"
        ...
        "    (xcquinox/pipeline/losses.py:937,964) which index into mol_names by\n"
        ...
        "    return pipeline.MoleculeSpec.from_dict(\n"
        ...
        "                'density_weight': 0.1,\n"
        "            }\n"
        "            spec = pipeline.TrainingSpec.from_dicts(\n"
    ...
    cells.append(_md(
        ...
        "Cited values are pulled from `xcquinox.pipeline.eval_probes`; see that\n"
    ...
    cells.append(_md(
        ...
        "Generates the six subset-sweep figures and headline.json from the eval_df.csv files\n"
        "of the trained specs, and two more figures with two tables where the held-out probe\n"
        "evaluations exist (`tools/analysis/step7_post_processing.py`, writing under\n"
        "`tools/analysis/figures_step7_post_processing/`).\n"
    ))
    cells.append(_code(
        "import sys\n"
        "import subprocess\n"
        ...
        "    sys.executable,\n"
        "    str(REPO / 'tools' / 'analysis' / 'step7_post_processing.py'),\n"
...
def main(output_path: str | Path | None = None) -> None:
    ...
    Cell ids are set to ``cell_NN`` in order, as the step 4 to 6 builders do, so two builds
    of the same source are byte-identical and the tracked notebook can be held to the
    builder's output.
    """
