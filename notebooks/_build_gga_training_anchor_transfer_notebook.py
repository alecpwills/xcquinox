"""Generator for notebooks/gga_training_anchor_transfer.ipynb.
...
All geometries and AE references come from the W4-11 set (Karton et al. 2011),
read from the per-species ``struc.xyz`` files of its distribution.
...
# The step counts of the three stages (2026-04-28: 1000 / 100 / 250, from 200 / 45 / 125,
# which were too short for the V_xc residual to drop visibly under the L3 and L5 signal).
DEFAULT_PRETRAIN_N_STEPS = 1000
DEFAULT_TRAIN_N_STEPS_SHORT = 100
DEFAULT_TRAIN_N_STEPS_LONG = 250
...
def build_cell_02_imports():
    ...
    source = (
        ...
        ")\n"
        "# SolverConfig + enums are not re-exported on xcquinox.pipeline; import from submodule.\n"
        "from xcquinox.pipeline.solver import SolverConfig, SolverMode, FeaturePolicy\n"
        "# The reference SCFs of the data cells run at the pipeline's pins (the small-density\n"
        "# cutoff, the XC block size, the ERI path): pyscf 2.14 prunes no grid point by default\n"
        "# and the pipeline pins the previous default, so an unpinned SCF here would put the\n"
        "# CCSD density on a grid the precompute does not build.\n"
...
def build_cell_03_constants(checkpoint_base: str = DEFAULT_CHECKPOINT_BASE,
                            pretrain_n_steps: int = DEFAULT_PRETRAIN_N_STEPS,
                            train_n_steps_short: int = DEFAULT_TRAIN_N_STEPS_SHORT,
                            train_n_steps_long: int = DEFAULT_TRAIN_N_STEPS_LONG):
    """Section 1 Cell 3 -- the knobs. The three step counts are parameters so the
    end-to-end smoke of the notebook can train a few steps of every stage rather than
    the production counts; the defaults are the notebook's own."""
    source = f"""# Step-6 knobs. All training / eval cells read from these.
CHECKPOINT_BASE          = {checkpoint_base!r}
PRETRAIN_N_STEPS         = {int(pretrain_n_steps)}
PRETRAIN_SKIP_IF_EXISTS  = True
TRAIN_N_STEPS_SHORT      = {int(train_n_steps_short)}
TRAIN_N_STEPS_LONG       = {int(train_n_steps_long)}
...
_arch_objs = [pipeline.get_architecture(n) for n in ARCH_NAMES]
...
for atom_symbol, spin in PRETRAIN_ATOMS:
    ...
    if need_dm:
        ...
        # meaningless idempotency_error for open-shell (E3 review).
...
def build_cell_08_pretrain_loop():
    ...
    Always qualifies as ``pipeline.PretrainSpec`` and ``pipeline.run_pretrain``.
...
for arch_name in ARCH_NAMES:
    if PRETRAIN_SKIP_IF_EXISTS and _pretrain_checkpoints_exist(arch_name):
        ...
        continue
    spec = pipeline.PretrainSpec(
        arch=pipeline.get_architecture(arch_name),
    ...
    pipeline.run_pretrain(spec, progress_callback=_cb)
"""
    return new_code_cell(source)
...
if os.path.isfile(_npz) and os.path.isfile(_meta) and _npz_has_vxc_ref(_npz):
    ...
    with _capped_blas_threads(4):
        if os.path.isfile(_npz) and os.path.isfile(_meta):
            ...
            _mf_pbe = dft.RKS(_mol); _mf_pbe.xc = "pbe"; _mf_pbe.grids.level = GRID_LEVEL; pin_reference_scf(_mf_pbe)
        ...
        _oep_spec = pipeline.MoleculeSpec(
        ...
        for _tier_name, _tier_kw in _OEP_TIERS:
            ...
            try:
                _oep = pipeline.run_oep_inversion(
        ...
        if _oep is not None and _oep.converged:
            ...
            pipeline.save_vxc_ref(_oep, _npz, dm_target=dm_ao, method="ccsd")
        else:
...
for _name, _atom_str, _spin in ATOM_SPECS:
    ...
    _mf_pbe = dft.UKS(_mol); _mf_pbe.xc = "pbe"; _mf_pbe.grids.level = GRID_LEVEL; pin_reference_scf(_mf_pbe)
    _mf_pbe.kernel(); E_pbe = float(_mf_pbe.e_tot)
...
_arch_objs = [pipeline.get_architecture(_n) for _n in ARCH_NAMES]
_desc_keys = set()
...
for _ms in (H2O_spec, C2H2_spec, H_spec, O_spec, C_spec):
    mol_data_by_name[_ms.name] = pipeline.precompute_fixed_density_data(
        _ms,
...
# The training stage refuses a pretrain checkpoint without a fidelity
...
# the five training entities above without their external data (the
...
# example pretrains on the samples of four atoms to demonstrate the flow, so
# enforcement is off with the reason on the record: the verdict is computed,
# written and printed, the on-node gate proceeds on the waiver, and a run
# certified this way cannot enter the record layers (validate_run, the
...
_CERT_OVERRIDE_REASON = (
    "step-6 example: the networks are pretrained on the samples of four atoms "
    "to demonstrate the training flow; the certificate is recorded, not enforced")
...
for arch_name in ARCH_NAMES:
    _cert = _fidelity.fidelity_certificate(
        _certificate_config(arch_name), RUN_DIR, arch_name,
...
for _arch in ARCH_NAMES:
    for _loss in LOSS_NAMES:
        for _solver in SOLVER_LABELS:
            ...
            _specs_group1.append(pipeline.TrainingSpec.from_dicts(
                arch=pipeline.get_architecture(_arch),
                loss_name=_lname,
    ...
    for _loss in LOSS_NAMES:
        for _solver in SOLVER_LABELS:
            ...
            if _loss == "L1_B":
                ...
                raise ValueError(f"unknown loss label: {_loss!r}")
            _specs_group2.append(pipeline.TrainingSpec.from_dicts(
                arch=pipeline.get_architecture(_arch),
                loss_name=_lname,
    ...
    for _loss in LOSS_NAMES:
        for _solver in SOLVER_LABELS:
            ...
            if _loss == "L1_B":
                ...
                raise ValueError(f"unknown loss label: {_loss!r}")
            _specs_group3.append(pipeline.TrainingSpec.from_dicts(
                arch=pipeline.get_architecture(_arch),
                loss_name=_lname,
...
def build_cell_21_training_loop():
    ...
    source = (
        ...
        ")\n"
        "\n"
        "# Host (CPU) OOM signatures: a large-basis XLA/LLVM compile can exhaust\n"
        "# node RAM (std::bad_alloc / 'Cannot allocate memory' / pthread_create\n"
        "# failed). Recognizing them lets a CPU-fallback attempt be classified as\n"
        "# a genuine OOM rather than a permanent failure.\n"
        "_CPU_OOM_MARKERS = (\n"
        "    'std::bad_alloc',\n"
        "    'bad_alloc',\n"
        ...
        "        return True\n"
        "    # Host-allocator OOM (a large-basis CPU compile exhausts node RAM).\n"
        "    if any(m in text for m in _CPU_OOM_MARKERS):\n"
        "        return True\n"
        "    # Signal evidence with no captured marker: SIGKILL (-9 / 137, the OS\n"
        "    # OOM-killer) or SIGABRT (-6 / 134, a C++ std::bad_alloc -> abort()).\n"
        ...
        "    xcquinox.pipeline._train_one_spec` imports the xcquinox.pipeline package\n"
        ...
        "    \"\"\"\n"
        "    cmd = [sys.executable, '-m', 'xcquinox.pipeline._train_one_spec', spec_path]\n"
        "    env = None\n"
...
def build_cell_22_loss_curves():
    ...
    ``xcquinox.pipeline.train._save_artifacts`` -- NOT from
...
training molecules with ``pipeline.run_test`` to produce per-molecule metrics
...
def build_cell_25_main_sweep():
    ...
    build a ``TestSpec`` and call ``pipeline.run_test``. Artifacts land under
...
def _ensure_pbe_atom_total(Z):
    ...
    _mf.xc = "pbe"; _mf.grids.level = GRID_LEVEL; pin_reference_scf(_mf)
    _mf.kernel()
...
for _idx, _spec in enumerate(_all_specs):
    ...
    _test_spec = pipeline.TestSpec.from_dicts(
        arch=pipeline.get_architecture(_arch),
...
def _eval_baseline_model(model, arch_name, kind):
    ...
    save_trained_checkpoint(_ckpt_path, model, pipeline.get_architecture(arch_name))
    _spec = pipeline.TestSpec.from_dicts(
        arch=pipeline.get_architecture(arch_name),
    ...
    pipeline.run_test(_spec)
    jax.clear_caches(); gc.collect()

for _arch in ARCH_NAMES:
    # (1) pretrained: load xnet/cnet from pretrain/ but skip training.
    _arch_cfg = pipeline.get_architecture(_arch)
    _xnet_sk, _cnet_sk = pipeline.create_network_pair(_arch_cfg, seed=0)
    ...
    _pretrained_model = pipeline.AlecGGAModel.from_arch(
    ...
    _random_model = pipeline.AlecGGAModel.from_arch(_arch_cfg, seed=12345)
...
def build_cell_32_transfer_primary_eval():
    ...
    ``pipeline.run_test``, and aggregates the ``per_molecule.json`` outputs
...
# specs, runs pipeline.run_test on each transfer molecule, aggregates to tidy
...
# _run_transfer_eval (which calls pipeline.run_test under the hood) defined
...
from xcquinox.pipeline.checkpoint_class import load_trained_checkpoint
...
def _load_full_model_from_ckpt(_ckpt_path, _arch_name):
    ...
    _arch_cfg = pipeline.get_architecture(_arch_name)
...
`notebooks/_build_gga_training_anchor_transfer_notebook.py` -- never hand-edit.
""")
...
def main(
    ...
    output_path: str = "notebooks/gga_training_anchor_transfer.ipynb",
    pretrain_n_steps: int = DEFAULT_PRETRAIN_N_STEPS,
    train_n_steps_short: int = DEFAULT_TRAIN_N_STEPS_SHORT,
    train_n_steps_long: int = DEFAULT_TRAIN_N_STEPS_LONG,
) -> nbformat.NotebookNode:
    """Assemble the step-6 notebook. The step counts are the notebook's own by default;
    the end-to-end smoke passes small ones."""
    arch_names = arch_names or DEFAULT_ARCH_NAMES
    ...
    cells = [
        ...
        build_cell_03_constants(checkpoint_base=checkpoint_base,
                                pretrain_n_steps=pretrain_n_steps,
                                train_n_steps_short=train_n_steps_short,
                                train_n_steps_long=train_n_steps_long),
