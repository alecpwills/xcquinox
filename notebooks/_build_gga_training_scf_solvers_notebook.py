"""Generator for notebooks/gga_training_scf_solvers.ipynb.
...
# ``xcquinox.pipeline.ARCHITECTURES``.
DEFAULT_ARCH_NAMES = (
    "deep",
    "deep_attn",
    "deep_cusp",
...
def build_cell_04_arch_table():
    ...
    """
    source = """# Print the deep-only registered architectures from pipeline.ARCHITECTURES.
# Step 5 focuses on deep variants only (8 total).
# Fields printed: name, depth, nodes (hidden size), attention flag, descriptors.
_deep_names = [n for n in pipeline.ARCHITECTURES.keys() if n.startswith("deep")]
...
def build_cell_05_arch_names(arch_names: tuple[str, ...] | None = None):
    ...
    if arch_names is None:
        arch_binding = (
            'ARCH_NAMES = [n for n in pipeline.ARCHITECTURES.keys() '
...
def build_cell_15_mol_specs():
    """Section 4 Cell 15 -- construct three pipeline.MoleculeSpec objects.
...
mol_data_list = []
for ms in mol_specs:
    md = pipeline.precompute_fixed_density_data(
        ms,
...
def build_cell_19_training_loop():
    ...
    source = (
        ...
        "            [sys.executable, '-m', 'xcquinox.pipeline._train_one_spec', _spec_path],\n"
...
for loss_name in BAL_LOSS_NAMES:
    for bal_label, bal_cfg in BALANCING_CONFIGS.items():
        ...
        _lkw = {**LOSS_KWARGS_BASE[loss_name], "solver_config": cfg}
        bal_specs.append(pipeline.TrainingSpec.from_dicts(
            arch=pipeline.get_architecture(BAL_ARCH),
            loss_name=loss_name,
            molecules=tuple(mol_specs),
            targets=targets,
            atom_energies=atom_energies,
...
for arch_name in ARCH_NAMES:
    ...
    if (os.path.isfile(f"{pretrain_src}/xnet.eqx")
        ...
        model = pipeline.AlecGGAModel.from_arch(
    ...
    if not os.path.isfile(random_model_path):
        ...
        model = pipeline.AlecGGAModel.from_arch(arch, seed=42)
...
if _ch4_item is None:
    ...
    for _arch_name in ARCH_NAMES:
        ...
        _ch4_mol_data_by_arch[_arch_name] = pipeline.precompute_fixed_density_data(
            _ch4_spec,
    ...
    from xcquinox.pipeline.descriptors import assemble_descriptor_features as _asm_feat
        ...
        for _arch, _ckpt in _ginfo['ckpts']:
            ...
            try:
                _model = load_trained_checkpoint(
                    _ckpt, pipeline.AlecGGAModel.from_arch(_arch_cfg),
                )
...
if _loss_b not in LOSS_NAMES:
    ...
    for solver_label in SOLVER_LABELS:
        ...
        if not os.path.isfile(ckpt):
            ...
            continue
        _arch_config = pipeline.get_architecture(_best_arch)
        _model = load_trained_checkpoint(ckpt, pipeline.AlecGGAModel.from_arch(_arch_config))
        _dm_nn = pipeline.oneshot_dm_prediction_fast(_model, mol_data_list[2])
        _delta = _dm_nn - dm_hf
...
if _loss_c not in LOSS_NAMES:
    ...
    for solver_label in SOLVER_LABELS:
        ...
        _arch_config = pipeline.get_architecture(_best_arch)
        _model = load_trained_checkpoint(ckpt, pipeline.AlecGGAModel.from_arch(_arch_config))
        _rho_nn = pipeline.oneshot_grid_density(_model, mol_data_list[2])
...
for _arch_name in ARCH_NAMES:
    ...
    for _loss_name in LOSS_NAMES:
        ...
        _n_attempted += 1
        try:
            _model = load_trained_checkpoint(_ckpt, pipeline.AlecGGAModel.from_arch(_arch_cfg))
        except Exception as _e:
...
# name still identifies the cell semantically.
def main(
    output_path: str,
    *,
...
if __name__ == "__main__":
    main("notebooks/gga_training_scf_solvers.ipynb")
