...
\begin{frame}{Scope}
  ...
  size & deep\_2x8, deep\_attn\_2x8, deep\_3x16, deep\_attn\_3x16 & deep\_3x16 11/11, deep\_attn\_3x16 9/11, the 2x8 pair 0/11 & \path{run_20260902T145245Z} \\
...
\begin{frame}{1. Model: the architectures}
  ...
  \toprule
  Architecture & Registry key & Last layer & Rung, parent & Hidden & Heads & Extra \\ \midrule
  ...
  deep0\_3x16 & deep\_3x16 & zeroed & GGA, PBE & 3 x 16 & none & none \\
  deep0\_attn\_3x16 & deep\_attn\_3x16 & zeroed & GGA, PBE & 3 x 16 & 4 & none \\
  deep0\_cusp\_3x16 & deep\_cusp\_3x16 & zeroed & GGA, PBE & 3 x 16 & none & cusp \\
  deep0\_cusp\_mgga\_3x16 & deep\_cusp\_mgga\_3x16 & zeroed & meta-GGA, SCAN & 3 x 16 & none & cusp, $\alpha$ \\
  \bottomrule
  \end{tabular}
  \end{center}
  \scriptsize
  \begin{itemize}
    ...
    \item deep\_3x16 and deep0\_3x16 have the same layers, inputs and parameter count [code: config.py:512, 577--580]; deep0\_3x16 differs only in a zero-initialized final layer, $F_x = F_c = 1$ at initialization [code: config.py:137--141, networks.py:287--290] (its two other flags act on descriptors it does not carry). After pre-training both are PBE clones differing only in their fitted weights: the two columns are replicates of one architecture under two starts; their combined held-out verdicts agree at every subset size, and on the energy leg alone they differ at 3 and 4 points (Sec. 5).
  \end{itemize}
\end{frame}
\slidenote{\textbf{The names:} the family word states the initialization of the last layer (deep: Glorot; deep0: zeroed, so pre-training starts from the LDA), then the descriptor and attention tokens, then the size; the registry keys the runs are filed under are in the table. \textbf{Why deep\_3x16 and deep0\_3x16 look alike:} the registry (config.py) builds medium (shown as deep\_3x16) as depth 3, width 16 with no flags, and deep\_3x16 (shown as deep0\_3x16) as depth 3, width 16 with the three "deep-family" flags set. Of the three, \texttt{dm\_entropy\_intensive} acts on the density-matrix descriptor and \texttt{descriptor\_log\_transform} on descriptor columns, neither of which deep0\_3x16 carries, so on this axis only \texttt{zero\_init\_final\_layer} has an effect: the last layer's weights and bias start at zero, so the untrained network returns exactly the LDA ($F = 1$), whereas deep\_3x16 starts from a Glorot initialization (with $F_x$ about $2.6\times10^{-4}$ off 1 on average). Pre-training then fits both to PBE; the certificate shows both pass (deep\_3x16 mean $|\Delta AE|$ 0.162, deep0\_3x16 0.114 kcal/mol). The registry keys come from its history (the "deep" family was introduced as 4 x 32 networks with these flags, the 3 x 16 twins added later); the shown names state the one difference. \textbf{Do the deep networks do worse?} No: deep0\_3x16 and deep\_3x16 are below PBE on the combined objective at the same subset sizes (1, 7, 12, 15, 18, 26) and above it at the same sizes (2 to 6); on energy alone deep0\_3x16 is below PBE at 3 and 4 points where deep\_3x16 is not. The comparison is between two clones of one architecture, which is a replicate rather than an architecture contrast. The deep\_2x8 pair (2 x 8) and the descriptor families (cusp, meta-GGA) are the actual architecture axes, and their cells are still queued.}
...
\begin{frame}{3. Pre-training: targets and objective}
  \small
  Each network is fitted pointwise [code: pretrain.py:261--341, pretrain\_data\_gen.py] to reproduce its parent (PBE on the GGA rung, SCAN on the meta-GGA) on the parent's own densities of the DFS pre-training inventory plus every atom of the BH76 and W4-11 pools (39 systems GGA, 38 meta-GGA), with a synthetic $(r_s, s, \alpha)$ mesh [DFS21 SI] at 30 percent of the weight:
  \[
...
\begin{frame}{3. Pre-training: the loss curves}
  ...
  \begin{column}{0.5\linewidth}
    ...
    \begin{itemize}
      ...
      \item GGA clones end at $1.5\times10^{-8}$ to $9.5\times10^{-8}$ (exchange) and $6.7\times10^{-8}$ to $3.4\times10^{-6}$ (correlation); deep\_attn\_3x16 descends earliest and lowest, at the longest fit (28 h against 3.6 h for its plain twin).
      \item The SCAN clone (right) stops fifty to a hundred times higher after the same 20000 steps and fails the certificate (previous slide). Its four siblings are pending; the meta-GGA training array is held.
    \end{itemize}
  \end{column}
  \end{columns}
\end{frame}
\slidenote{Left: the seven certified GGA clones of the size and families runs on one panel pair; right: the meta-GGA clone (its certificate failed, so it is drawn from its run). The curves are the first term of the pre-training objective, the weighted mean squared pointwise residual, for the exchange network (left panel of each pair) and the correlation network (right panel), against the optimizer step on a log scale; the final value of each curve is in the legend. The regular spikes from step 5000 onward are an artifact of the validation procedure: every 50 steps the network is scored on the held-out rows and the best snapshot is reloaded, and the first step after a reload sits above the trend for one step. The GGA clones all reach residuals of order $10^{-8}$ to $10^{-6}$ in units of the squared enhancement-factor difference, which corresponds to sub-kcal/mol atomization errors (previous slide). The attention variants take much longer per step (28 h against 3.6 h) but reach the lowest residuals. The SCAN clone's curves are smooth and still decreasing slowly at 20000 steps; it does not diverge, it is simply far from converged to SCAN's accuracy, which is consistent with the meta-GGA form (an additional input, $x_\alpha$, and SCAN's more structured dependence on $\alpha$) needing more capacity or more steps than the GGA clones.}
...
\begin{frame}{3. Pre-training: the pre-trained networks against their parents}
  ...
  \begin{column}{0.5\linewidth}
    ...
    \scriptsize
    \begin{itemize}
      \item Left: the difference $F^{\rm NN} - F^{\rm PBE}$ of every GGA clone, exchange at $n = 1$ and correlation at $r_s = 2$, along $s$. Within $10^{-3}$ for $s < 2.5$, at most 0.015 at $s = 6$ (deep\_attn\_2x8 exchange); the three zero-init clones within $4.2\times10^{-3}$ everywhere.
      \item Right: the SCAN clone. $F_x(\alpha = 0)$ is 0.01 to 0.027 above SCAN for $s > 2.5$, $F_c(\alpha = 1)$ 0.005 to 0.033 below it from $s = 2$; same-sign offsets over wide $s$ ranges do not cancel between a molecule and its atoms.
    \end{itemize}
  \end{column}
  \end{columns}
\end{frame}
\slidenote{These figures show how closely each pre-trained network reproduces its parent along one-dimensional slices of the input space: the exchange enhancement factor as a function of $s$ at unit density, and the correlation factor as a function of $s$ at $r_s = 2$ (and, for the meta-GGA clone, at fixed $\alpha = 0$ and $\alpha = 1$, the two slices SCAN's paper uses). \textbf{The left figures} draw only the differences from PBE, one curve per architecture; the titles printed inside these PNGs read "pretrained corrections", a wording left from the plotting script, whereas the curves are the plain differences $F^{\rm NN} - F^{\rm PBE}$ of networks that were fitted to produce PBE, not to correct it (the correction to PBE is what the training of Sec. 4 produces). For $s$ below 2.5, where most of the energy-weighted grid points lie, every GGA clone is within $10^{-3}$ of PBE; the differences grow beyond $s = 3$, a region the pre-training set samples sparsely, but stay below 0.015. \textbf{The right figure} is the meta-GGA clone against SCAN, with the parent dashed and the network solid and the difference panels beside them: the clone is above SCAN in exchange at $\alpha = 0$ over a wide range of $s$ and below it in correlation at $\alpha = 1$, and offsets of one sign over wide ranges are exactly what does not cancel in an atomization energy (molecule minus atoms), which is why this clone fails the certificate while its pointwise residual is only fifty times larger than the GGA clones'.}
...
\begin{frame}{4. In-sample: the training losses (deep\_3x16)}
  ...
  \scriptsize
  \begin{itemize}
    \item SCF: 3 cycles from the converged PBE density, DFS mixing $D_{t+1} = (1 - a_t) D_t + a_t D^{\rm out}_t$, $a_t = 0.3^{t} + 0.3$ [DFS21 SI; code: solver.py:305--316], gradients through every cycle, the last mixed density scored. AdamW $10^{-3} \to 10^{-5}$ over the second half, weight decay $10^{-4}$, clip 1.0, 200 epochs, validation MAE on 47 reactions every 25 epochs [cfg \texttt{hyperparams:}].
    \item The reaction channel falls in every cell, by 3x (deep0\_3x16 at 18) to 190x (deep\_attn\_3x16 at 2); the density channel by 1.3x to 4.6x, both settled within 25 epochs. The potential channel, absent from the DFS objective, rises from seven points upward (deep\_3x16 at 18: 3.9e-5 to 4.3e-5).
    \item Validation MAE: the 2-to-6-point cells of deep\_3x16 and deep0\_3x16 rise after the first check (deep\_3x16 at 2: 11.7 to 32.7 kcal/mol, best at epoch 25); the cells from seven points hold 6 to 9 kcal/mol; deep\_attn\_3x16's small cells fall (37 to 7 kcal/mol).
  \end{itemize}
\end{frame}
\slidenote{\textbf{The figure} has eight panels for the deep\_3x16 architecture, one curve per subset size (dark: one point, yellow: twenty-six), each a per-epoch mean on a log scale: the total loss, then the five channels at their trained weights (the five add up to the total), the validation reaction-energy error at each 25-epoch check with the chosen (validation-best) epoch starred, and the learning rate. \textbf{The SCF:} the network's energies and densities are self-consistent, but only three Kohn-Sham cycles are run from the converged PBE density, with the DFS linear mixing schedule (the new density is mixed into the old with a weight $a_t$ that decays from 1.3 toward 0.3); gradients flow through all three cycles. Three cycles are enough when the functional is close to PBE and do not converge when it is far from a fixed point, which is one of the reasons the held-out density comparison is asymmetric (Sec. 5); a 25-cycle variant is running. \textbf{AdamW} is the optimizer with decoupled weight decay; the learning rate is held for the first half of the run and decayed linearly over the second. \textbf{What the curves show:} the reaction channel is the one the training reduces by orders of magnitude; the density channel falls modestly and settles within 25 epochs; the potential channel rises for the larger subsets, i.e. the potential term and the energy terms pull in different directions once many molecules are supervised. The validation error (reactions not in the training subset) separates the cells into two families: the cells trained on 2 to 6 points start overfitting after 25 epochs (their validation error rises), while the one-point cell and every cell from seven points holds a stable 6 to 9 kcal/mol; the validation-best checkpoint is the epoch with the lowest validation error, so the small cells are read at epoch 25.}
...
\begin{frame}{4. In-sample: the trained enhancement factors}
  ...
  \scriptsize
  \begin{itemize}
    \item Exchange, every architecture and subset: the trained $F_x$ is below PBE by 0.02 to 0.06 near $s = 0.8$ and above it from $s \approx 1.4$, by up to 0.18 (deep\_3x16), 0.13 (deep\_attn\_3x16), 0.14 (deep0\_3x16) at $s = 2.5$ to 3; the one-molecule cell moves least.
    \item Correlation at $r_s = 2$: between 0.6 and 1.0 up to $s = 2$ (PBE falls to 0.02 by $s = 3$), then cell-dependent: deep\_3x16 at 7 to 26 points decays to 0.4 to 0.6, at 2 and 3 points rises to 1.1 to 1.4; every deep\_attn\_3x16 cell but the one-molecule one rises above 1 (up to 1.9 at $s = 6$).
  \end{itemize}
\end{frame}
...
\begin{frame}{5. Held-out: the set and the metrics [Goe17; DFS21 Eqs. 20, 21]}
  ...
  \begin{itemize}
    ...
    \item The harmonic mean is set by its smaller leg: with $\gamma\varepsilon_n \approx 10$ kcal/mol here, 1 kcal/mol of $E$ moves $\mathcal{ED}$ by 0.4 to 0.7, the whole density spread by at most 1.5. Code: evaluation.py:217; make\_ablation\_arch\_figure.py:3715--3750, 4840.
  \end{itemize}
\end{frame}
...
\begin{frame}{5. Held-out: the full set beside the set without the recurring density tail}
  ...
  \begin{itemize}
    ...
    \item The density tail recurs: over the 33 cells, bn (the largest ratio at every size), rkt17, cloo, bn3pi, cn, h2, rkt06 and cs are above 1.5x PBE in at least half of the cells carrying them. Without them (right) the deep\_3x16 column from seven points is within 4 percent of PBE's $\varepsilon_n$, deep\_attn\_3x16 below it at 1, 2, 5, 6, 7, 12, 15, deep0\_3x16 at 15 and 26, deep0\_attn\_3x16 at 1; the $\mathcal{ED}$ verdicts change only for deep0\_3x16 at three points.
...
\begin{frame}{6. Pending work}
  \small
  \begin{itemize}
    \item Cells still queued: deep\_attn\_3x16 at 12 to 26, deep0\_attn\_3x16 and deep0\_cusp\_3x16 at every subset, the deep\_2x8 pair.
    ...
    \item Converged-SCF re-evaluation of the 28 finished cells (DIIS at $10^{-8}$ Ha) and the CCSD T1 diagnostics for a model-free multireference list: two cluster jobs pending.
