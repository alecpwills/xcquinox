...
methods box (`tools/analysis/make_ablation_arch_figure.py`) and its
bibliography (`docs/references.bib`); the reference *numbering* below is
this document's own.
...
**How the pieces fit.** *Descriptors* (`xcquinox/pipeline/descriptors.py`, `metagga.py`) compute the
...
publishable figures. `dfs_demo.py` is a thin orchestration layer over `xcquinox.pipeline`; the sections
...
All helpers live in `dfs_demo.py` and wrap the production `xcquinox.pipeline` APIs (so the demo config is
...
6. **Train** -- `xcquinox.pipeline.run_training(spec)`, differentiating through the SCF; live per-step
...
- **The full pipeline:** for the complete DFS pool + BH76/IP13 channels + V_xc supervision, use the
  cluster harness (`xcquinox.pipeline.cluster`) with the `dfs_step7` config; its CCSD-reference generator
...
Bibliographic details are the repo's `docs/references.bib` and its
methods box.
