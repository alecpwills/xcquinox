...
negligibly (< 0.1 kcal/mol). See the companion README for the physics and `xcquinox.pipeline.orientation_lock`.
...
import dfs_demo
from xcquinox.pipeline import run_training, run_test

# Shared cross-figure rung palette, so the meta-GGA-vs-SCAN story reads the same
# here and in the cluster ablation figures. arch_style lives in ../../tools/analysis; load
# it defensively -- the notebook still runs (matplotlib defaults) if it moves.
try:
    sys.path.insert(0, os.path.abspath(os.path.join(os.getcwd(), "..", "..", "tools", "analysis")))
    import arch_style
except Exception:
    arch_style = None
...
- To adapt: change `ARCH_NAMES` or pass a custom `ArchitectureConfig` to `build_dfs_training_spec`;
  ...
  V_xc supervision, use the cluster harness (`xcquinox.pipeline.cluster`) with the `dfs_step7` config; the
  CCSD reference generator there is `xcquinox.pipeline.external_refs.precompute_all` (adds the OEP V_xc
  cascade).
...
def main(output_path=None):
    """Write the notebook to ``output_path`` (``train_dfs_density.ipynb`` beside this file by
    default), with ``cell_NN`` ids so two builds of the same source are byte-identical."""
    for idx, cell in enumerate(nb["cells"]):
        cell["id"] = f"cell_{idx:02d}"
    out = output_path or os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      "train_dfs_density.ipynb")
