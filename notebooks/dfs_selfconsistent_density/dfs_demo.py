...
Thin orchestration over ``xcquinox.pipeline``. Every spec this module builds is
...
from xcquinox.pipeline import benchmark_refs, get_architecture
...
from xcquinox.pipeline.config import PretrainSpec, TestSpec, TrainingSpec
...
from xcquinox.pipeline.orientation_lock import DEFAULT_STRENGTH as _OL_DEFAULT_STRENGTH
...
def generate_ccsd_density_refs(mol_specs: Sequence, *, refs_dir: str, basis: str,
    ...
    Reuses :func:`xcquinox.pipeline.benchmark_refs.generate_one` (converged HF ->
...
def pretrain_atoms_for(mol_specs):
    ...
    if missing:
        raise KeyError(
            ...
            "xcquinox.pipeline.dfs_pool.ATOMIC_GROUND_STATE_SPIN (with a citation)."
        )
