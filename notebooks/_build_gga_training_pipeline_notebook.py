"""Generator for notebooks/gga_training_pipeline.ipynb.
...
this module; the tests beside it state the contract cell by cell.
...
# ``DEFAULT_ARCH_NAMES`` synchronized with ``xcquinox.pipeline.ARCHITECTURES``.
DEFAULT_ARCH_NAMES = (
    "shallow",
    "shallow_attn",
    "medium",
...
`xcquinox.pipeline` subpackage. All loss, network, training, and evaluation
...
the notebook. Step 4 delegates every step to `xcquinox.pipeline`:

- `pipeline.PretrainSpec` / `pipeline.run_pretrain` -- pretraining phase
- `pipeline.TrainingSpec.from_dicts` / `pipeline.run_training` -- main training phase
- `pipeline.TestSpec.from_dicts` / `pipeline.run_test` -- evaluation phase
...
def build_cell_04_arch_table():
    ...
    """
    source = """# Print all 12 registered architectures from pipeline.ARCHITECTURES.
...
print(_header)
print("-" * len(_header))
for _name in pipeline.ARCHITECTURES.keys():
    _cfg = pipeline.get_architecture(_name)
...
def build_cell_05_arch_names(arch_names: tuple[str, ...] | None = None):
    ...
    if arch_names is None:
        arch_binding = "ARCH_NAMES = list(pipeline.ARCHITECTURES.keys())"
    else:
...
# cutoff, the XC block size, the ERI path): pyscf 2.14 prunes no grid point by default and
# the pipeline pins the previous default, so an unpinned SCF here would put the CCSD
# density on a grid the precompute does not build.
...
spec = pipeline.PretrainSpec(
    arch=pipeline.get_architecture({arch_name!r}),
...
def build_cell_10_pretrain_parity():
    ...
    (``xcquinox/pipeline/pretrain.py:69-88``): rho, sigma, dm columns (if any),
...
for row, arch_name in enumerate(ARCH_NAMES):
    arch = pipeline.get_architecture(arch_name)
    skel_xnet, skel_cnet = pipeline.create_network_pair(arch)
...
1. ``{name}.npz`` -- holds **only** the three whitelisted keys that
   ``xcquinox.pipeline.data`` accepts: ``dm_target``, ``rho_ref_grid``,
...
def build_cell_14_mol_specs():
    """Section 4 Cell 14 -- construct three pipeline.MoleculeSpec objects.
...
for _arch_name in ARCH_NAMES:
    for _desc in pipeline.get_architecture(_arch_name).materialize_descriptors():
...
mol_data_list = [
    pipeline.precompute_fixed_density_data(m, required_keys=_mol_data_required_keys)
    for m in mol_specs
]
...
def build_cell_17_training_specs(loss_names=None):
    """Section 5 Cell 17 -- build 72 pipeline.TrainingSpec objects.
...
for arch_name in ARCH_NAMES:
    for loss_name in LOSS_NAMES:
        specs.append(pipeline.TrainingSpec.from_dicts(
            arch=pipeline.get_architecture(arch_name),
            loss_name=loss_name,
...
def build_cell_18_training_loop():
    ...
    For a parallel variant using pipeline.build_training_jobs +
    pipeline.run_workers, see spec line 88 -- not emitted here.
...
# pipeline.build_training_jobs + pipeline.run_workers, see spec line 88 -- not emitted here.
#
...
try:
    for spec in specs:
        ...
        if TRAIN_SKIP_IF_EXISTS and _training_model_exists(spec):
            ...
            continue
        pipeline.run_training(spec, progress_callback=_train_cb)
        _spec_bar.update(1)
...
`pipeline.TestSpec.from_dicts(...)` + `pipeline.run_test(spec)`. Four metrics are
...
Cell 22 below runs `pipeline.run_test` over the same `(arch, loss)` product as
...
def build_cell_22_test_loop():
    ...
    for loss_name in LOSS_NAMES:
        ckpt_dir = f"{CHECKPOINT_BASE}/train/{arch_name}/{loss_name}"
        test_spec = pipeline.TestSpec.from_dicts(
            arch=pipeline.get_architecture(arch_name),
...
for loss_name in ("B_atomization_plus_dm", "D1_delta_ae", "D2_delta_ae_plus_dm"):
    ...
    arch_config = pipeline.get_architecture(best_arch)
    model_template = pipeline.AlecGGAModel.from_arch(arch_config)
...
if not model_bindings:
    ...
    dm_nn_B = pipeline.oneshot_dm_prediction_fast(model_B, mol_data_list[2]) if model_B is not None else None
    dm_nn_D1 = pipeline.oneshot_dm_prediction_fast(model_D1, mol_data_list[2]) if model_D1 is not None else None
    dm_nn_D2 = pipeline.oneshot_dm_prediction_fast(model_D2, mol_data_list[2]) if model_D2 is not None else None
...
def build_cell_27_density_histograms():
    ...
    source = """from xcquinox.pipeline.checkpoint_class import load_trained_checkpoint
...
if not _c_d3_losses_present:
    ...
    for ax, loss_name in zip(_axes_flat, _c_d3_losses_present):
        ...
        rho_nn = pipeline.oneshot_grid_density(model, mol_data_list[2])
...
new_mol_spec = pipeline.MoleculeSpec(
    name="CH4",
...
new_test_spec = pipeline.TestSpec.from_dicts(
    arch=pipeline.get_architecture(best_arch),
...
# pipeline.run_test(new_test_spec)
"""
    return new_code_cell(source)
...
def build_cell_32_new_mol_comparison():
    ...
    in Cell 31, runs ``pipeline.run_test`` per combination, and renders a 1x3
    ...
    * Density RMSE with the PBE-vs-HF density RMSE reference line from the
    ...
    """
    source = """# Sweep every (arch, loss) combination, run pipeline.run_test on the new molecule,
...
exists on disk, runs `pipeline.run_test` on the new molecule from Cell 31, and
...
def main(
    ...
    *,
    ...
    # (``ARCH_NAMES = list(pipeline.ARCHITECTURES.keys())`` / ``LOSS_NAMES = (...)``
...
if __name__ == "__main__":
    main("notebooks/gga_training_pipeline.ipynb")
