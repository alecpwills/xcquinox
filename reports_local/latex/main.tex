...
3 solvers, 3 training-step budgets, training on H$_2$O alone (group 1) or
H$_2$O+C$_2$H$_2$ (groups 2--3), 90 specs per origin).
...
population means 4.25$\times$; per-arch range 2.95--4.74$\times$ in step 5 and
...
\citep[their \S 2, eqs.~(26)--(27)]{LiebOxford1981}; \citet{PerdewBurkeErnzerhof1996}
...
\end{quote}
Our network (\texttt{xcquinox/alec/networks.py:16--44}, the
\texttt{\_AlecLOB} class) enforces $F_x \le 1.804$
...
directly: the per-volume integrand $n\,\varepsilon_X^{\text{unif}}(n)\, F_x(s) \propto
...
Neural-network exchange--correlation functionals are a
...
This section specifies the experimental apparatus exactly as it would be
needed to reproduce the runs: \S\ref{sec:methods}--\S\ref{sec:methods_arch_loss}
define the network architecture, the Lieb--Oxford clamp, the unweighted vs.\
integration-weighted pretrain loss, and the architecture / loss / solver
enumeration shared by both step subsets;
\S\ref{sec:methods_molecule_rationale} explains the chemical reasoning
behind the training and transfer molecule sets;
\S\ref{sec:methods_training_eval} fixes the optimizer, learning-rate
schedule, basis set, integration grid, and reference data sources.
...
\texttt{self.limit$\,*\,$jax.nn.sigmoid(x$\,-\,$jnp.log(self.limit$\,-\,$1.0))$\,-\,$1.0}
...
augmentations on a base \texttt{deep} MLP: optional
density-matrix-statistics descriptors (\texttt{dm\_statistics};
correlation-indicator features) (\texttt{\_dm}), optional cusp-correction features
...
task-loss steps; step 6 trains on H$_2$O alone (group~1) or
H$_2$O+C$_2$H$_2$ (groups~2,~3) for 100 (groups~1,~2)
...
computed at the W4-or-higher \textit{ab initio} level; the underlying W4
method reproduces Active Thermochemical Tables atomization energies for
35 benchmark species with $3\sigma < 1$~kJ/mol;
...
correlation by definition). He uses an explicit non-Chakravorty
reference: Chakravorty 1993's Table~VIII reports 3- through
10-electron CAS-IP correlation increments $\Delta E_c'(N,Z)$, and their
Table~X provides the corresponding total correlation energies for
$N=3$--$18$. The 2-electron correlation energy is taken from a
separate theoretical determination (\citealp{ChakravortyEtAl1993}
ref.~[1]; based on Davidson \emph{et al.}\ 1991, Phys.~Rev.~A
\textbf{44}, 7071).
...
The Results section is organised by training phase: \S\ref{sec:results_pretrain}
covers the pretraining phase (relevant to all four workflows);
\S\ref{sec:results_step5} covers the H$_2$O-only step-5 optimization
(8 archs $\times$ 3 losses $\times$ 3 solvers, 72 specs per origin); and
\S\ref{sec:results_step6} covers the H$_2$O+C$_2$H$_2$ step-6 optimization
...
through \S\ref{S:sec:lo_audit}). The cross-workflow headline numbers
referenced throughout this section are summarised in
Tab.~\ref{tab:headline_4way}.
...
to move under any re-weighting. The per-architecture $F_x$ vs $F_c$
ratio scatter (Fig.~\ref{S:fig:fx_fc_correlation}) shows a weak positive
correlation ($r = +0.56$) with substantial spread: simpler architectures
land in the both-channels-benefit quadrant while descriptor-rich
variants land in the $F_x$-tighter-but-$F_c$-worse quadrant.
...
(Fig.~\ref{fig:headline_bestae}). The integration-origin best spec on
H$_2$O is
...
$\sim$59,500$\times$ below PBE on H$_2$O. The $11.9 / 4.25 \approx 2.80\times$
...
Group-3 MAE rows: mean over the 6 (arch, solver) cells per loss on the
multi-mol training set (computed in
\texttt{notebooks/analysis/comparison\_lib.py:1066} as
\texttt{ae[ae.group=="group3"].groupby("loss")["mae"].mean()}).}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../step6_integration_analysis/figures/fig2_density_vs_energy_tradeoff.png}
...
H$_2$O (paired comparison in Tab.~\ref{tab:step6_paired}): step 5's loss set (no $V_{xc}$ residual) cannot reach below
...
The downstream best-AE ratio on the full training set
(H$_2$O+C$_2$H$_2$, 2.86$\times$) is comparable to but does \emph{not}
exceed the pretrain-$F_x$ ratio (3.06$\times$), so there is no
aggregate compounding factor analogous to step 5's 2.80$\times$.
However, restricting the step-6 evaluation to H$_2$O alone (the same
metric step 5 uses) gives a best-AE ratio of $0.003112 / 0.000426 \approx
7.3\times$, a 2.4$\times$ multiplier on top of the pretrain ratio --- so
the head start \emph{does} still compound on the per-molecule axis;
the apparent saturation is an averaging artifact of including
C$_2$H$_2$. In addition, the best $\rho$-RMSE of $1.23\!\times\!10^{-4}$
e/bohr$^3$ is achieved by \emph{group-1} (H$_2$O-only training)
\texttt{deep\_combined\_attn} with \texttt{L3\_balanced\_vxc} and
\texttt{oneshot} solver --- the narrowest training set + an explicit
$V_{xc}$ residual --- not by the broader group-3 training data.

A second nuance: per-loss group-3 MAE ratios reveal that the integration
win is loss-family-specific, not uniform. \texttt{L2\_C\_anchor} drops
to 0.099~kcal/mol on the integration origin (the only spec family that
clears chemical accuracy in-distribution; 3.39$\times$ tighter than
unweighted), while the four other loss families
(\texttt{L1\_B} 0.91$\times$, \texttt{L3\_balanced\_vxc} 0.67$\times$,
\texttt{L4\_balanced\_vxc\_anchor} 0.78$\times$,
\texttt{L5\_gradnorm\_vxc} 0.82$\times$) all favour the unweighted
origin at the median. The 2.86$\times$ best-AE win is therefore driven
essentially by L2 alone; the per-loss bar chart is
Fig.~\ref{S:fig:step6_per_loss_ratio}. The per-loss
...
2.95--4.74$\times$ in step 5 (mean 3.98$\times$ per-arch, 4.25$\times$ for the population
means); the two step-6 architectures give 3.00$\times$ and 3.13$\times$ (population-mean
ratio 3.06$\times$). This is the most consistent cross-workflow result
...
e/bohr$^3$. Step 6 added $V_{xc}$-aware losses (L3, L4); their best density-RMSE is
...
\citet{Kepp2017}, who shows the trend Medvedev highlighted is specific to compact
$1s^2$ and $1s^2 2s^2$ atomic systems and is not chemically relevant for typical
bonding regimes.
...
potentials which differ markedly from the exact ones'' phrasing is quoted by
\citet{Kepp2017} (his ref.~2) and traces to \citet{CruzLamBurke1998}; the broader
Burke--Perdew--Ernzerhof body of work of the late 1990s
(adiabatic-connection method, hybrid functionals, etc.) provides the wider
context for the energy-vs-density tradeoff principle. The Medvedev 2017 across-decades thesis
...
of \citet[Table~2 and \S\S 5.2--5.4]{ChenGradNorm2018}; the same paper
also uses $\alpha = 0.12$ for synthetic regression toy problems with
$T = 2$ and $T = 10$ tasks (\S 4, Fig.~2), so the value is task-specific and not asserted
...
% written up in the methods section, cite the OEP sources flagged "TO DOWNLOAD"
