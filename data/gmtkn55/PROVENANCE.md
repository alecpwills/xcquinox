...
The full GMTKN55 benchmark is checked out beside this file, under `data/gmtkn55/`
(ignored; 153 MB), for local reference-value extraction; `xcquinox.pipeline.full_benchmark_pools`
reads it from there, or from the directory `XCQUINOX_GMTKN55_DIR` names, only when the two
JSON caches under `xcquinox/pipeline/data/` are rebuilt.

- **Upstream:** https://github.com/grimme-lab/GMTKN55
- **Pinned commit:** `354c3ded3371f43545a33e7077866f4254e04917` (2025-09-17)
- Re-fetch, into the directory that holds this file (a clone refuses a directory that is
  not empty, so the checkout is copied into it):

```bash
git clone https://github.com/grimme-lab/GMTKN55.git /tmp/GMTKN55
git -C /tmp/GMTKN55 checkout 354c3ded3371f43545a33e7077866f4254e04917
cp -a /tmp/GMTKN55/. data/gmtkn55/
```
...
`xcquinox/pipeline/dfs_pool.py` stores the 3 Dick training BH76 reaction energies from
