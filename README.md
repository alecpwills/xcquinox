...
declaration writes no upper bound of its own. The details, and how to run the tests and
build the documentation, are in [docs/install.md](docs/install.md).
...
and tables. The walkthrough, from a one-minute local training run to a full sweep, is the
[user guide](docs/user_guide.md); the code-flow diagrams are in
[docs/architecture](docs/architecture/pipeline_training_flow.md).
...
The same commands are reachable as `python -m xcquinox.pipeline.cluster`. Pulling the
results and regenerating the figures is the
[pull-and-figures runbook](docs/pipeline/pull_and_figures.md); the cluster paths and the
queue conventions are in [hpcjobs/SEAWULF_RUNBOOK.md](hpcjobs/SEAWULF_RUNBOOK.md).
...
cite; `notebooks/` holds the worked examples, each generated by its builder.

**The record.** [docs/open_items.md](docs/open_items.md) records what was deliberately not done, with the
measurement that forced the deferral.
