...
def test_train_dm_loss():
    ...
    xcT = xce.train.xcTrainer(xc, optax.adamw(1e-4), DMloss, steps=1, logfile='test_dm_log')
...
def test_train_holo_loss():
    DMloss = xce.loss.DM_HoLu_loss()
    xcT = xce.train.xcTrainer(xc, optax.adamw(1e-4), DMloss, steps=1, logfile='test_holo_log')
    init_loss = DMloss(xc, ao_evals[0], gws[0], dms[0], eris[0], mo_occs[0], hcs[0], ss[0], ogds[0], hologaps[0], alpha0=0.7,
...
def test_train_rho_loss():
    DMloss = xce.loss.DM_HoLu_loss()
    xcT = xce.train.xcTrainer(xc, optax.adamw(1e-4), DMloss, steps=1, logfile='test_rho_log')
    init_loss = DMloss(xc, ao_evals[0], gws[0], dms[0], eris[0], mo_occs[0], hcs[0], ss[0], ogds[0], hologaps[0], alpha0=0.7,
