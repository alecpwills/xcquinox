...
Usage
-----
    python -m xcquinox.pipeline._train_one_spec <spec_path>

Loads a serialized ``TrainingSpec`` from disk, runs ``pipeline.run_training``
...
``subprocess.run([sys.executable, "-m", "xcquinox.pipeline._train_one_spec",
...
``BalancingConfig``), a pickle read through the checkpoint-class loader,
which maps the class paths of specs pickled before the subpackage's
rename. The spec file is generated and consumed by the same trusted
codebase in the same process tree -- it is never read from an
untrusted source.
...
def _flush_on_signal() -> None:
    ...
    try:
        ...
        from xcquinox.pipeline import train as _train
...
def _load_spec(path):
    ...
    # The spec is read through the checkpoint-class loader, which maps the class
    # paths of specs pickled before the subpackage's rename; the file is produced
    # by this codebase in the same process tree, never by another source.
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "padding ON without rebuilding it. See xcquinox.pipeline.padding."
        ),
    )
    args = parser.parse_args(argv)

    # Route JAX before ANY import that pulls it in. This MUST run before the
    # xcquinox.pipeline import below (which transitively imports jax).
    if args.device == "cpu":
    ...
    try:
        # Lazy import keeps startup fast if anything fails before we need pipeline.
        ...
        t0 = time.time()
        pipeline.run_training(spec, progress_callback=cb)
        sys.stdout.write(
        ...
        # the shared hard exit (xcquinox/pipeline/cluster/_exit.py), which runs no
...
if __name__ == "__main__":
    ...
    # verdict (see xcquinox/pipeline/cluster/_exit.py); imported here since
