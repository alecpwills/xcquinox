...
One more value the reference SCF's grid depends on is a release default
rather than the process's memory: ``small_rho_cutoff``, the density
threshold ``initialize_grids`` prunes the grid with at the first SCF cycle
(``prune_small_rho_grids_`` keeps a point when ``|rho| w > cutoff /
n_points`` on the initial-guess density). PySCF's Kohn-Sham class default
was 1e-7 through release 2.11 and is 0 (no pruning) from 2.14, and the
records, the closed-shell fixtures, the pinned point counts and the
tail-row conditioning of the descriptors all stand on the pruned grids.
:func:`pin_small_rho_cutoff` holds the value at
:data:`REFERENCE_SMALL_RHO_CUTOFF`, under which the pruned grids of the two
releases are identical bit for bit (coordinates and weights; H2O at
def2-svp levels 1 and 3, the O and H atoms at level 1: 9304, 30632, 4504
and 2336 points, against 10128, 33704, 5184 and 2472 at the 2.14 default).
...
(``reference_xc_blksize``), the integral path (``reference_eri_path``) and
the density cutoff (``reference_small_rho_cutoff``) so a mismatch is
visible. (Before the auxiliary loops were pinned, the
...
#: Density threshold the reference SCF prunes its integration grid with at
#: the first cycle (``KohnShamDFT.small_rho_cutoff``; a point is kept when
#: ``|rho| w > cutoff / n_points`` on the initial-guess density). PySCF's
#: class default through release 2.11, and 0 from 2.14 (no pruning); held
#: at this value so every record is a quadrature of the pruned grid
#: whatever the release's default (module docstring: the pinned grids of
#: the two releases are identical bit for bit).
REFERENCE_SMALL_RHO_CUTOFF: float = 1e-7
...
class ReferencePins(NamedTuple):
    ...
    eri_path: str
    #: Density threshold the grid was pruned with at the first cycle
    #: (:data:`REFERENCE_SMALL_RHO_CUTOFF`), or None for a mean-field
    #: without a grid (Hartree-Fock).
...
def pin_xc_block_size(mf, blksize: int = REFERENCE_XC_BLKSIZE) -> int | None:
    ...
    block_loop.__doc__ = (
        ...
        f"(xcquinox.pipeline.pyscf_determinism.pin_xc_block_size).")
...
def pin_small_rho_cutoff(mf, cutoff: float = REFERENCE_SMALL_RHO_CUTOFF
                         ) -> float | None:
    """Fix the density threshold ``mf`` prunes its integration grid with.

    ``initialize_grids`` drops, once, at the first SCF cycle, the points
    where the initial-guess density is negligible
    (``prune_small_rho_grids_``: a point is kept when ``|rho| w >
    mf.small_rho_cutoff / n_points``), and the class default that value
    falls back to moved from 1e-7 to 0 between PySCF 2.11 and 2.14. The
    value is set on the instance, before the first ``get_veff`` builds the
    grid; a grid built explicitly beforehand is pruned on no release, and
    the pin then records a value that did not act.

    Returns the value applied, or None when ``mf`` has no
    ``small_rho_cutoff`` (Hartree-Fock: no grid, nothing to prune).
    """
    if not hasattr(mf, "small_rho_cutoff"):
        return None
    mf.small_rho_cutoff = float(cutoff)
    return float(cutoff)
...
def pin_reference_scf(mf) -> ReferencePins:
    ...
    Applies :func:`pin_small_rho_cutoff` first, before any grid exists,
    then :func:`pin_xc_block_size` (both no-ops returning None on a
    Hartree-Fock object) and :func:`pin_eri_path`, and reads the thread
    count, so the caller can put all four into the record's metadata.
    """
    cutoff = pin_small_rho_cutoff(mf)
    return ReferencePins(xc_blksize=pin_xc_block_size(mf),
                         threads=reference_thread_count(),
                         eri_path=pin_eri_path(mf),
                         small_rho_cutoff=cutoff)
