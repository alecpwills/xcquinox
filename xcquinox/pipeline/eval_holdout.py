...
``tools/analysis/local_reeval.py`` into a place the cluster eval
(``xcquinox.pipeline.cluster._eval_one_spec``) can also import.

The local CLI (``tools/analysis/local_reeval.py``) re-exports these
...
# three-cycle warm channels are not (density review, 2026-09-07).
...
def density_errors_for_record(model, md: Dict[str, Any], *,
    ...
    The NN channel reuses :class:`~xcquinox.pipeline.evaluation.DensityRMSEMetric`
    ...
    if md.get("rho_ref_grid") is None:
        return none_result
    import xcquinox.pipeline.evaluation as evaluation
...
def require_precomputed_species(precomputed, requested) -> None:
    ...
    cell whose every reaction is NaN (a cell whose precompute
    had refused every reference file).
...
def precompute_holdout(
    ...
    *,
    ...
    for i, (name, spec) in enumerate(mol_specs.items(), start=1):
        t1 = time.time()
        try:
            out[name] = pipeline.precompute_fixed_density_data(
...
def evaluate_holdout(model, mol_data: Dict[str, Any],
    ...
    * ``FULL`` -> the self-consistent ``pipeline.run_scf(...).total_energy`` (and the
    ...
    * ``ONESHOT`` / ``FIXED_J`` / ``None`` -> the one-shot
      ``pipeline.fixed_density_total_energy(model, mol_data[name])`` on ρ_PBE.
    ...
    """
    import xcquinox.pipeline as pipeline
    from xcquinox.pipeline.solver import run_scf, SolverMode
    ...
    for name, md in mol_data.items():
        try:
            if use_scf:
                ...
                e = float(pipeline.fixed_density_total_energy(model, md))
        except Exception as exc:  # noqa: BLE001
...
def assert_channel_not_sliced(spec_dir: Path, eval_subdir: str) -> None:
    ...
    (``XCQUINOX_HELDOUT_SPECIES_SLICE``), not the
    216-reaction BH76 + W4-11 pool the architectures are
...
def load_trained_model(training_spec, model_path: Path):
    ...
    ``xcquinox.pipeline.oneshot.fixed_density_total_energy`` based on the
