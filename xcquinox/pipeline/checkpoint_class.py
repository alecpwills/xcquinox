...
#: The module path older pickles name for the pipeline's classes, and the current one. Every
#: TrainingSpec the cluster harness pickled beside a checkpoint (``specs/spec_NNNN.spec``)
#: names its classes under the old path; the campaigns' artifacts are read through
#: :class:`RenamingUnpickler`, never rewritten, and this pair is the one place the tree
#: keeps the old name besides the development log.
RENAMED_PACKAGE = ("xcquinox.alec", "xcquinox.pipeline")


class RenamingUnpickler(pickle.Unpickler):
    """An unpickler that resolves the class paths of objects pickled before the
    subpackage's rename: a class stored under ``xcquinox.alec.<module>`` is looked up as
    ``xcquinox.pipeline.<module>``; every other path is looked up as stored."""

    def find_class(self, module, name):
        old, new = RENAMED_PACKAGE
...
def load_pickle(file):
    """``pickle.load`` from a binary file object, through :class:`RenamingUnpickler`.

    The pickles read through it are the harness's own artifacts, written by this package on
    the cluster or on this machine (the materialized specs, the run logs), never data from
    another source; a pickle from elsewhere would execute what it carries.
    """
