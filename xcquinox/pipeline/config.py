"""xcquinox.pipeline.config: architecture, feature, and pipeline spec dataclasses.
...
class ArchitectureConfig:
    ...
    def describe(self) -> dict:
        ...
        derived from THIS configuration (``xcquinox.pipeline.arch_names``: the
        ...
        for f in fields(self):
            ...
            if f.name in ("descriptors", "x_constraints", "c_constraints"):
                ...
                out[f.name] = value
        from xcquinox.pipeline.arch_names import derive_display_name
...
ARCHITECTURES = {
    ...
    # is capacity. Probe-only until measured: they sit on no submitted group's
    # axis, so they carry no campaign cell.
...
class TrainingSpec:
    ...
    # neutral (see xcquinox.pipeline.padding). Threaded from HyperParams by spec_builder.
    ...
    def validate(self) -> None:
        """Raise ValueError if spec is inconsistent."""
        # Deferred local import, xcquinox.pipeline.losses does not exist until
        # Task 4.1 ships.
        from xcquinox.pipeline.losses import LOSS_REGISTRY, list_losses
...
class TestSpec:
    ...
    def validate(self) -> None:
        ...
        # Deferred local import, xcquinox.pipeline.evaluation does not exist
        # until Task 5.3 ships.
        from xcquinox.pipeline.evaluation import METRIC_REGISTRY, list_metrics
