"""xcquinox.pipeline.train -- custom training loop for AlecGGAModel.
...
from xcquinox.pipeline.checkpoint_class import (ModelClassMismatch,
                                            ...
                                            stage_class_record,
                                            stale_temporaries)
from xcquinox.pipeline.config import TrainingSpec, ArchitectureConfig
...
from xcquinox.pipeline.procmem import read_rss_gb
...
def build_optimizer(
    *,
    ...
    DFS (the adam->adamw switch is the generalization-gap fix),
    not accidental parity breaks.
...
def _require_fidelity_certificate(pretrain_checkpoint: str) -> None:
    ...
    The certificate (``xcquinox.pipeline.cluster.fidelity``) is the only
    ...
    parent in atomization energies, larger than
    ...
    raise ValueError(
        ...
        f"xcquinox.pipeline.cluster.fidelity <run_dir> <arch_idx>`, or set "
...
def _require_matching_model_class(pretrain_checkpoint: str, arch) -> None:
    ...
    beside the checkpoint, ``pretrain_metadata.json``,
    and a mismatch is refused with both states named. A record
...
def _build_validation_data(spec):
    ...
    if seed_source == "scan":
        ...
        from xcquinox.pipeline.data import missing_seed_cache_files
...
def _validation_reaction_mae(model, val_mol_data, val_reactions,
    ...
    if energy_fn is None:
        from xcquinox.pipeline.oneshot import total_energy_for_solver
...
def _remove_resume_set(checkpoint_dir) -> None:
    """Delete the whole ``resume_*`` set: each file, its class record, and any
    temporary sibling an interrupted atomic write left behind.

    Two callers, for the two reasons a set stops being usable.
    :func:`_finalize_completion` calls it because ``model.eqx`` has superseded
    the set; :func:`_load_resume_checkpoint` calls it when it REFUSES the set
    as another model class, where leaving it is what would let a later partial
    write re-label its files (the records and the leaves are separate files,
    so a set that survives a refusal can be updated one file at a time). A
    refused set can never be loaded again in that directory: the run that was
    refused starts fresh and overwrites it. Deleting an absent file is
    tolerated, so both callers are idempotent.
    """
...
def _load_resume_checkpoint(checkpoint_dir, *, model_skeleton,
    ...
    def _require(path):
        """The class check, with the refused set discarded on refusal."""
        try:
            return require_matching_class(path, want_class,
                                          what="resume checkpoint")
        except ModelClassMismatch as exc:
            _remove_resume_set(checkpoint_dir)
            raise ModelClassMismatch(
                f"{exc} The refused resume set and its class records have "
                f"been removed from {os.fspath(checkpoint_dir)!r}, so no "
                "later partial write can re-label them as this class."
            ) from exc
...
def _finalize_completion(checkpoint_dir, *, early_stopped, epochs_run) -> None:
    ...
    goes with it (:func:`_remove_resume_set`), so no record outlives the
...
def _filter_loss_kwargs(loss_kwargs_dict, target_loss_name):
    ...
    import inspect
    from xcquinox.pipeline.losses import LOSS_REGISTRY
...
def _run_gradnorm_loop(spec, model, batch, loss, progress_callback):
    ...
    for step in range(spec.n_steps):
        ...
        # intermediates between calls so peak memory is about ONE forward+
