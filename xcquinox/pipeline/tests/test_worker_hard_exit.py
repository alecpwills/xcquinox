...
CLUSTER_DIR = REPO_ROOT / "xcquinox" / "pipeline" / "cluster"
WORKERS_DIR = REPO_ROOT / "xcquinox" / "pipeline" / "workers"
EXIT_MODULE = CLUSTER_DIR / "_exit.py"

#: The entry points the matrix's smoke and the campaign job graph actually
...
NAMED_ENTRY_POINTS = {
    ...
    "workers/evaluation_worker.py", "workers/eval_holdout_worker.py",
    # rendered job stages that live one level up from the two directories
    "pipeline/benchmark_refs.py",
...
#:
#: One SLURM-launched reader is deliberately absent.
#: ``hpcjobs/dfs6311_c2_ref_probe.py`` imports neither ``xcquinox`` nor JAX --
#: it reads cached ``.npz`` files with numpy and writes nothing -- so no
#: teardown handler is registered and an abort costs a re-read. Reaching the
#: helper through ``xcquinox.pipeline.cluster._exit`` would import JAX and PySCF
#: into that process (measured: 1686 modules against a numpy-only base), i.e.
#: it would create the exposure it is meant to close.
EXTRA_ENTRY_FILES = (
    ...
    REPO_ROOT / "tools" / "analysis" / "precompute_scan_pool.py",
    REPO_ROOT / "tools" / "analysis" / "precompute_nonempirical_pool.py",
)
...
def test_atexit_abort_does_not_hijack_the_exit_status(tmp_path, rc=7):
...
def _ends_in_the_shared_hard_exit(relpath):
...
def test_every_entry_point_ends_in_the_shared_hard_exit():
    """Every entry point, in one case: the set is read from the tree, so a new worker
    is held to the idiom without a registration here."""
    entry_points = sorted(_entry_points())
    assert entry_points, "no entry point found"
    for relpath in entry_points:
        _ends_in_the_shared_hard_exit(relpath)
