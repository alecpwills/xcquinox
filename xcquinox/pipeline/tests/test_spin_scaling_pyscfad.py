...
import xcquinox.pipeline.solver_pyscfad as solver_pyscfad
...
from xcquinox.pipeline.solver_pyscfad import (
    _build_pyscfad_mf, _make_pipeline_eval_xc, _maybe_metagga_ao_grad,
    _maybe_rung35_proj_ao, _maybe_rung35ms_proj_ao,
    _reassemble_features_on_grid, install_functional)
...
def _callback_exc(model, md, mf, mol, dm, holder):
    ...
    cb = _make_pipeline_eval_xc(model, model.descriptors, md, FeaturePolicy.FROZEN,
                            feature_holder=holder)
    install_functional(mf, cb, "GGA")
    holder["offset"] = 0
...
def test_pyscfad_uks_callback_refuses_a_holder_without_per_channel_blocks():
    ...
    def callback(holder):
        return _make_pipeline_eval_xc(model, model.descriptors, md,
