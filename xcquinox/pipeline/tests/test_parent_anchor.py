...
from xcquinox.pipeline.config import ArchitectureConfig, MoleculeSpec, anchored
...
from xcquinox.pipeline.oneshot import uks_zeta
...
#: (measured in the design review),
#: nine orders under the certificate's 1.0 mHa.
_RHO_FLOOR = 1e-10

#: Representative architectures per rung: the GGA rungs are anchored to PBE and
#: the meta-GGA rung to SCAN (``parents.parent_for_arch``), and the two parents
...
def test_anchored_meta_gga_networks_return_scan_at_initialization(arch_name):
    ...
    the OH rows.
    """
    from xcquinox.pipeline.networks import _raw_indicator
...
def test_an_anchored_zeta_blind_correlation_network_is_refused():
    ...
    supported.
    """
    arch = anchored(dataclasses.replace(pipeline.get_architecture("deep_3x16"),
...
def test_o1_anchored_exchange_path_equals_libxc_spin_polarized_pbe(symbol, spin):
    ...
    from xcquinox.pipeline.oneshot import split_exc_energy_uks
    from xcquinox.pipeline.tests.parent_adapter import gga_rho_row
...
def _anchored_spec_builder_cfg(tmp_path, parent_anchor=True):
    ...
    from xcquinox.pipeline.tests.test_cluster_spec_builder import _make_cfg
...
def test_the_loader_refuses_an_anchor_state_mismatch(tmp_path, monkeypatch,
    ...
    base = dataclasses.replace(pipeline.get_architecture(arch_name),
                               use_polarized_correlation=True)
