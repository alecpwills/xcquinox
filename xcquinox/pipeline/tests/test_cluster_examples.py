...
def test_v8_arms_pretrain_on_the_protocol_set_and_the_certified_atoms_alone():
    """Every campaign-1 arm resolves the pretraining set the protocol binds
    plus every free atom its own fidelity certificate gates on, at the charge
    and spin the certificate measures it at, and nothing beyond them; the
    pinned arms are every campaign-1 file on disk.

    Oracle: the DFS pretraining inventory at the GGA level
    (``dfs_pretrain_set.dfs_pretrain_records``), the pools' free atoms
    (``pretrain_data_gen.pool_atom_systems``) and the single-centre systems
    of ``fidelity.build_oracle_set`` for each swept architecture. The two
    inventories are the resolver's own sources, so what is pinned here is the
    composition -- which inventories, what the explicit block adds, and the
    de-duplication -- and not their contents, which their own test modules
    hold; the certificate's atoms are the one source the resolver does not
    read. The set under test is assembled the way the datagen stage assembles
    it: the protocol keywords of ``_datagen._protocol_keywords``, narrowed to
    the ones ``resolve_pretrain_systems`` accepts, at the parent density the
    arm's architectures resolve to, asserted to be the GGA parent since the
    meta-GGA inventory is a different roster. Free atoms are compared as
    ``(symbol, charge, 2S)`` rather than by name: the certificate and the
    generator spell an ion's name differently, and a name says nothing about
    the spin the atom is fit at.

    The certificate closes its free-atom set over every element a molecule
    of the inventory dissociates into, which brings in sodium, carried by
    neither inventory; the explicit ``pretrain.atoms`` block is what fits
    it, six of its seven entries de-duplicating against the inventories and
    changing nothing. Only the atom half of the certificate's gate is pinned:
    its three fixed molecules sit at the pool geometry, one of them absent
    from the inventory, by the decision recorded beside the certificate. The
    comparison is shown live with an atom none of the three sources supplies:
    the same keywords widened by a free potassium atom resolve to a set the
    guard refuses.
    """
    pytest.importorskip("yaml")
    import inspect
    from xcquinox.pipeline import pretrain_data_gen as pdg
    from xcquinox.pipeline.cluster import _datagen, fidelity
    from xcquinox.pipeline.dfs_pretrain_set import dfs_pretrain_records

    paths = _require_v8_files()
    on_disk = sorted(entry for entry in os.listdir(_campaign_configs_dir())
                     if entry.startswith("dfs_step8.")
                     and entry.endswith(".yaml")
                     and not entry.endswith(".local.yaml"))
    assert on_disk == sorted(_V8_ARMS), (
        f"the campaign-1 files on disk are {on_disk} against the pinned arms "
        f"{sorted(_V8_ARMS)}; an arm outside the list states its pretraining "
        "set to nothing")

    def _free_atom(system):
        """``(symbol, charge, 2S)`` of a single-centre system, else None."""
        geometry = str(system.atom).strip()
        if ";" in geometry or len(geometry.splitlines()) != 1:
            return None
        return (geometry.split()[0], int(system.charge), int(system.spin))

    inventory = [pdg.normalize_system(record)
                 for record in dfs_pretrain_records("gga")]
    inventory += list(pdg.pool_atom_systems())
    inventory_names = {system.name for system in inventory}
    inventory_atoms = {_free_atom(system) for system in inventory} - {None}
    assert "Na" not in inventory_names and "K" not in inventory_names

    selecting = set(
        inspect.signature(pdg.resolve_pretrain_systems).parameters)
    for name, path in paths.items():
        cfg = load_grid_config(path)
        parents = sorted({reference for _polarized, reference
                          in _datagen._required_data_specs(cfg)})
        assert parents == ["pbe"], (
            f"{name}: the swept architectures resolve the parent densities "
            f"{parents}; the roster below is the PBE parent's GGA inventory")
        certified = set()
        for arch in _V8_ARCHS:
            for system in fidelity.build_oracle_set(cfg, arch):
                triple = _free_atom(system)
                if triple is not None:
                    certified.add(triple)
        assert ("Na", 0, 1) in certified, sorted(certified)
        assert not any(symbol == "K" for symbol, _c, _s in certified)

        def _accounted(system):
            """A free atom is one of the inventories' or the certificate's,
            at that charge and spin; a molecule is one of the inventories'."""
            triple = _free_atom(system)
            if triple is None:
                return system.name in inventory_names
            return triple in inventory_atoms or triple in certified

        keywords = {key: value for key, value
                    in _datagen._protocol_keywords(cfg.pretrain).items()
                    if key in selecting}
        widened = pdg.resolve_pretrain_systems(
            reference_xc=parents[0],
            **{**keywords,
               "atoms": tuple(keywords.get("atoms", ())) + (("K", 1),)})
        assert any(not _accounted(system) for system in widened), (
            f"{name}: a free K atom added to the arm's own protocol keywords "
            "is accounted for, so the comparison that follows accepts a set "
            "it has to refuse")
        resolved = pdg.resolve_pretrain_systems(reference_xc=parents[0],
                                                **keywords)
        beyond = sorted((system.name, _free_atom(system))
                        for system in resolved if not _accounted(system))
        assert not beyond, (
            f"{name}: {beyond} are carried by none of the three sources at "
            "that charge and spin")
        missing = sorted(inventory_names
                         - {system.name for system in resolved})
        assert not missing, (
            f"{name}: {missing} are in the protocol's inventories and not in "
            "the set")
        fitted = {_free_atom(system) for system in resolved} - {None}
        unfit = sorted(certified - fitted)
        assert not unfit, (
            f"{name}: {unfit} are certified at that charge and spin and never "
            "fit")


def test_the_energy_weight_probe_carries_the_arms_pretraining_atoms():
    """The probe's config-less fallback atom list is the block every tracked
    DFS-domain configuration states, entry for entry and in order, the
    campaign-1 arms among them: a change to the block reaches the probe or
    fails here, and a campaign that diverges from the others re-points the
    probe or fails here.

    Oracle: the literal in ``hpcjobs/probe_pretrain_energy_weight.py``, read
    off the script's syntax tree without importing it, against the raw
    ``pretrain.atoms`` block of every ``dfs_step*.yaml`` in the tracked
    configuration directory that carries one.
    """
    pytest.importorskip("yaml")
    import ast
    _require_v8_files()
    directory = _campaign_configs_dir()
    probe = os.path.join(os.path.dirname(directory),
                         "probe_pretrain_energy_weight.py")
    ...
    for node in tree.body:
        if isinstance(node, ast.Assign) and any(
                getattr(target, "id", None) == "CAMPAIGN_ATOMS"
                for target in node.targets):
            literal = tuple(tuple(entry)
                            for entry in ast.literal_eval(node.value))
    assert literal is not None, "the probe no longer states CAMPAIGN_ATOMS"

    compared = []
    for entry in sorted(os.listdir(directory)):
        if not (entry.startswith("dfs_step") and entry.endswith(".yaml")) \
                or entry.endswith(".local.yaml"):
            continue
        flat = _flat_yaml(os.path.join(directory, entry))
        block = tuple((key.rsplit(".", 1)[1], int(value))
                      for key, value in flat.items()
                      if key.startswith("pretrain.atoms."))
        if not block:
            continue
        assert block == literal, (entry, block, literal)
        compared.append(entry)
    assert set(_V8_ARMS) <= set(compared), compared
