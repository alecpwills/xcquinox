...
These lock the fixes for the dfs6311 compile-OOM: the training step
...
def test_defused_no_rejit_over_repeated_shapes():
    ...
    # the kernels this call compiles, cold: their own caches cleared, not the process's
    # every cache (jax.clear_caches() can abort a long session)
