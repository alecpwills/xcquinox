"""Unit test for tools/smoke_preflight_uks_oep.py.
...
Full-execution verification is the user-driven `python tools/smoke_preflight_uks_oep.py`
...
SCRIPT = REPO_ROOT / "tools" / "smoke_preflight_uks_oep.py"
...
def test_smoke_script_dry_run(tmp_path):
    ...
    # The bound is measured, not guessed: the dry run takes 1.5 s cold in a fresh
    # environment (2026-09-22, the cache-root venv on the current stack), and a 60 s
    # bound was exceeded once on a machine running the whole suite beside this test
    # (2026-09-20). 300 s is two hundred times the cold measurement.
    proc = subprocess.run(
