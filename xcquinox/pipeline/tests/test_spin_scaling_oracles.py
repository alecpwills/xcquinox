...
architecture-carrying case is parametrized over ``sorted(pipeline.ARCHITECTURES)``
...
from xcquinox.pipeline.descriptors import (
    ...
    doubled_spin_dm)
from xcquinox.pipeline.oneshot import split_exc_energy_uks, uks_zeta
...
from xcquinox.pipeline.tests.parent_adapter import (
    LibxcParentModel, gga_rho_row, mgga_rho_row, tau_from_alpha)
from xcquinox.pipeline.tests.test_closed_shell_byte_identity import (
    announce, assert_closed_shell_record_matches)
from xcquinox.pipeline.tests.test_solv01_split_xc import (
    _UKS_FD_SPECIES, _alpha_columns, _assert_uks_fd_consistency,
    _live_model, _md_with_descriptors)

# The per-architecture oracle set the cluster's workflow matrix runs on a node, one
# architecture at a time through ``cluster.workflow_matrix.oracle_selector``; deselected from
# the routine suite (the runner and the workflow run ``not slow and not oracle``), where the
# spin-scaling property is held by the precompute, solver and pyscfad modules on representative
# architectures.
...
_ARCHS = sorted(pipeline.ARCHITECTURES)
...
def _parent_of(arch_name):
    ...
    return _PARENT_LIBXC[resolve_parent_density(
        pipeline.get_architecture(arch_name), "auto")]
...
def test_o1_per_channel_ingredients_are_the_libxc_spin_polarized_ingredients(
    ...
    """
    from xcquinox.pipeline.metagga import compute_alpha, _RHO_FLOOR
    descriptors = (MetaGGAAlphaDescriptor(),)
    md = _precompute(symbol, spin, descriptors)
