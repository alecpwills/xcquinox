...
def _assert_uks_fd_consistency(model, md, arch_name, label, eps=_FD_EPS):
    ...
    from xcquinox.pipeline.models import _NN_TAIL_THRESHOLD
...
def test_uks_zeta_is_shared_by_energy_and_potential():
    ...
    """
    from xcquinox.pipeline import oneshot
    from xcquinox.pipeline.tests._source_scan import code_only
...
def _build_polarized_model(seed=0):
    ...
    arch = pipeline.ArchitectureConfig.from_spec(
...
def _ec_energy_polarized(model, D_a, D_b, features, ao_grid, ao_xyz,
    ...
    # (_RHO_TOT_FLOOR + where/stop_gradient).
...
def test_polarized_vc_finite_on_negative_density_tail():
    """Regression: diffuse-basis grid-tail quadrature noise (or
    ...
    from xcquinox.pipeline.oneshot import compute_vc_polarized_per_spin
...
def test_pyscfad_callback_closed_shell_reduction():
    ...
    from xcquinox.pipeline.solver_pyscfad import _make_pipeline_eval_xc
    ...
    md = _h2_rks_md()
    eval_xc = _make_pipeline_eval_xc(model, model.descriptors, md,
...
def _build_descriptor_model():
    ...
    arch = dataclasses.replace(
        pipeline.get_architecture("deep_combined_attn"),  # cusp + dm_statistics
        zero_init_final_layer=False)
    xnet, cnet = pipeline.create_network_pair(arch, seed=0)
    return pipeline.AlecGGAModel.from_arch(arch, xnet=xnet, cnet=cnet)
...
def _wiring_model(polarized: bool, seed: int = 0):
    ...
    """
    import dataclasses
    arch = dataclasses.replace(pipeline.get_architecture("deep_mgga_3x16"),
...
def test_wiring_fixed_density_total_energy_uses_the_channel_blocks(wiring_md):
    ...
    satisfy both bounds.
    """
    from xcquinox.pipeline.oneshot import fixed_density_total_energy, uks_zeta
