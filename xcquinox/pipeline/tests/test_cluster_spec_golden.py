...
The golden reference lives at ``xcquinox/pipeline/tests/data/notebook_spec_snapshot.json``
and is produced by the USER-run helper ``tools/capture_notebook_spec_snapshot.py``.
...
def test_golden_snapshot_atom_energies_match_domain():
    ...
    from xcquinox.pipeline.cluster.domain import ATOMIC_ENERGIES_CHAKRAVORTY
...
def _rebuild_spec_snapshot(grid_cell: dict) -> dict:
    ...
    same way ``tools/capture_notebook_spec_snapshot.py`` does.
    ...
    script_path = os.path.join(
        repo_root, "tools", "capture_notebook_spec_snapshot.py"
    )
...
def test_harness_reproduces_golden_spec_snapshot():
    ...
    until the user runs tools/capture_notebook_spec_snapshot.py. It FAILS
