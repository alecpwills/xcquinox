...
the archive directory::
    ...
    cp xcquinox/pipeline/tests/record_closed_shell_reference.py \\
       <archive-dir>/xcquinox/pipeline/tests/
    cd <archive-dir> && PYTHONPATH=<archive-dir> JAX_PLATFORMS=cpu \\
        python xcquinox/pipeline/tests/record_closed_shell_reference.py \\
        > <repo>/xcquinox/pipeline/tests/fixtures/closed_shell_reference_ae204537e.json
...
``closed_shell_reference_jax0102.json`` was produced by the same script run
from the repository root on the current stack (jax 0.10.2, pyscf 2.14.0, numpy
2.5.3) on the i7-5930K workstation, with the reference SCF's density cutoff
...
def main(argv=None):
    ...
    names = sorted(pipeline.ARCHITECTURES) if not args.arch else list(args.arch)
    unknown = [n for n in names if n not in pipeline.ARCHITECTURES]
    if unknown:
