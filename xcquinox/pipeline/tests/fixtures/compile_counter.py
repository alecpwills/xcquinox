...
def clear_kernel_caches(*modules) -> int:
    """Clear the compilation caches of the jitted kernels defined at the top level of
    ``modules`` (``eqx.filter_jit`` wrappers, whose ``jax.jit`` sits in ``_cached``, and
    plain ``jax.jit`` functions), so that a test counting compilations starts those kernels
    cold without clearing every cache the process holds. ``jax.clear_caches()`` over the
    thousands of kernels a long session accumulates aborted the interpreter inside the
    clear (a weak-reference removal during the clear); the kernels a test
    measures are the ones to reset. Returns the number of kernels cleared; a module set
    holding none is a caller error and raises ``ValueError``.
    """
    cleared = 0
    for module in modules:
        for name in dir(module):
            obj = getattr(module, name, None)
            for target in (obj, getattr(obj, "_cached", None)):
                if target is None:
                    continue
                clear = getattr(target, "clear_cache", None)
                size = getattr(target, "_cache_size", None)
                if callable(clear) and callable(size):
                    clear()
                    cleared += 1
                    break
    if cleared == 0:
        names = [getattr(m, "__name__", repr(m)) for m in modules]
        raise ValueError(f"no jitted kernel at the top level of {names}")
    return cleared
