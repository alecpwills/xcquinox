"""Opt-in shape-padding pass (:mod:`xcquinox.pipeline.padding`): results-neutral, and it
...
at compile time. The pass pads arrays to one common shape, strips the
...
from xcquinox.pipeline.train import _build_model, run_training
from xcquinox.pipeline.oneshot import scf_loss_tail_weights, total_energy_for_solver
from xcquinox.pipeline.tests.fixtures.molecules import h_atom, o_atom, h2_molecule
from xcquinox.pipeline.tests.fixtures.compile_counter import clear_kernel_caches
from xcquinox.pipeline import defused_grad, oneshot
from xcquinox.pipeline.defused_grad import defused_value_and_grad, _energy_trajectory_jit
from xcquinox.pipeline.padding import (common_pad_target, canonicalize_mol_data,
                                   PadTarget)
from xcquinox.pipeline.tests.test_defused_grad import _SC_FULL, _l5_group, _ARCH
...
def _count_compiles(thunk, needle=None):
    ...
    with the energy kernels of the de-fused and one-shot modules cold (their own caches
    cleared, not the process's every cache, which can abort a long session).
    ``needle`` restricts the count to compile messages naming a specific kernel, filtering
    out the many shape-independent scalar-op compiles."""
    clear_kernel_caches(defused_grad, oneshot)
    msgs = []
...
def test_padding_strips_the_reference_xc_provenance_string():
    ...
    from xcquinox.pipeline.padding import (_STRIP_KEYS, _pad_mol_data,
