...
def test_lob_ceiling_holds_for_extreme_pre_clamp_inputs():
    ...
    the LOB check of 2026-04, since retired). A regression of
    ...
    assert float(fx_pos.max()) <= 1.804 + 1e-9, (
        ...
        f"physical guarantee of the pipeline exchange network."
    )
...
def test_pipeline_xnet_matches_library():
    from xcquinox.pipeline.networks import AlecGGA_XNet
    ...
    pipeline_out = jax.vmap(xnet)(inputs)
    lib_out = jax.vmap(lib)(inputs)
    np.testing.assert_array_equal(
        np.asarray(pipeline_out), np.asarray(lib_out),
...
def test_pipeline_xnet_uses_real_attention():
...
def test_pipeline_arch_validation_rejects_bad_divisibility():
...
def test_cnet_spin_polarization_input_width_and_sensitivity():
    import jax.numpy as jnp
    from xcquinox.pipeline.networks import AlecGGA_CNet
