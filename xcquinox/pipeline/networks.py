"""xcquinox.pipeline.networks: Exchange and correlation enhancement networks.
...
from xcquinox.pipeline.parents import (
...
def _raw_indicator(alpha_column):
    ...
    saturated. Differentiable in the
...
class AlecGGA_XNet(eqx.Module):
    ...
    * ``"dfs"`` -- ``[x_s, *extras]`` with ``x_s = (1 - exp(-s^2)) ln(s + 1)``
    ...
    def _core(self, rho, sigma, features, raw=False):
        ...
        ``z_parent = lob_preimage(F_parent, limit)``,
        so ``gated = 0`` returns ``F_parent`` to round-off and
...
def create_network_pair(arch: ArchitectureConfig, seed: int = 42,
    ...
    if getattr(arch, "parent_anchor", False):
        ...
        # The 2026-08-25 record: the final layer is zero whatever
