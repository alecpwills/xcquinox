"""Generate a pretrain-data ``.npz`` for xcquinox.pipeline network pretraining.
...
def _mol_spec_for(system, basis, grid_level):
    """The :class:`~xcquinox.pipeline.config.MoleculeSpec` for one pretraining
...
def _dfs_pretrain_records(level):
    ...
    try:
        ...
        raise ImportError(
            ...
            "xcquinox.pipeline.dfs_pretrain_set (dfs_pretrain_records(level)); "
...
def spin_channel_exchange_rows(mol, mf, ao, dm_ab, *, descriptors=True,
    ...
    from xcquinox.pipeline.metagga import compute_alpha, compute_tau_from_dm
...
def _system_columns(system, basis, grid_level, *, reference_xc, polarized,
    ...
    # the threshold is pinned below at the precompute's value,
    # REFERENCE_SMALL_RHO_CUTOFF = 1e-7, the class default through pyscf 2.11
    # where 2.14's default of 0 would keep every point; on H2/STO-3G level 0
    # this removes points, on a single H it removes none). Replaying that
    # method with the same initial guess
...
def _degenerate_systems(systems, basis, grid_level):
    ...
    """
    from xcquinox.pipeline.cluster.fidelity import is_degenerate_atom
    flagged = []
