...
2. **Assembly + energy cotangents.** ``E`` is injected (see
   :mod:`xcquinox.pipeline.energy_override`) so the loss's channel assembly consumes
...
:mod:`xcquinox.pipeline.padding` pass; see that module for the mechanism.
"""
import jax
...
from xcquinox.pipeline.energy_override import (
    set_energy_override,
    reset_energy_override,
)
from xcquinox.pipeline.padding import pad_batch
...
def defused_value_and_grad(loss, model, batch, channel_weights, relative=False,
    ...
    :func:`xcquinox.pipeline.padding.common_pad_target`; when given, every molecule is
    canonicalized to that one common shape by the standalone
    :func:`xcquinox.pipeline.padding.pad_batch` pass (results-neutral) so the
