"""xcquinox.pipeline.losses: AlecLoss ABC, registry, 6 concrete losses.
...
from xcquinox.pipeline.energy_override import get_energy_override
...
def _compute_energies(model, mol_data, N, solver_config=None):
    ...
    the solver MODE via :func:`xcquinox.pipeline.oneshot.total_energy_for_solver`:
    ...
    * ``ONESHOT`` / ``FIXED_J`` / ``None`` -> the one-shot fixed-density
    ...
    if override is not None:
        ...
        # outside this graph and injected (see xcquinox.pipeline.defused_grad).
        return override
    return jnp.stack([
...
def _anchor_term(model, sample, weight: float) -> jnp.ndarray:
    ...
    from xcquinox.pipeline.pbe_anchor import pbe_anchor_loss
    from xcquinox.pipeline.oneshot import _nn_fx_local_uks
...
class L5GradnormVxcStep7(AlecLoss):
    ...
    Five GradNorm task channels (per spec §5b):
      loss_AE   - atomization-energy residuals (existing pipeline mechanism)
    ...
    in ``train._run_gradnorm_loop`` (xcquinox/pipeline/train.py:408) treats
    ...
    alpha=1.5 default at xcquinox/pipeline/balancing.py:55), while the
    ...
    def compute_components(self, model, batch, relative=False):
        ...
        # vxc + rho channels: existing pipeline mechanisms.
