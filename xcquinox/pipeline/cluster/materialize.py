"""xcquinox.pipeline.cluster.materialize: on-disk serialization of harness specs.

The harness expands a grid into ``TrainingSpec`` objects and must write them
to a shared filesystem so the per-task worker ``xcquinox.pipeline._train_one_spec``
