"""xcquinox.pipeline.cluster.submit: render sbatch scripts + submit the job graph.
...
def render_sbatch(kind: str, cfg, run_dir: str, array_max=None) -> str:
    ...
    Args:
        kind: ``"pretrain"``, ``"preflight"``, ``"train"`` or ``"eval"``. For
            ...
            ``cfg.cluster.device``.
        cfg: a :class:`~xcquinox.pipeline.cluster.grid_config.GridConfig`.
...
# ---------------------------------------------------------------------------
# submit_commands.txt: human-readable record
# ---------------------------------------------------------------------------
...
def _append_commands(run_dir: str, tag: str, lines: list[str]) -> None:
    ...
    file is a chronological record of every (would-be) submission.
    """
