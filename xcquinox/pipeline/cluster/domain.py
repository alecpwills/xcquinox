"""xcquinox.pipeline.cluster.domain: physics tables for the HPC training harness.

The cluster harness extracts step-7 spec-building from the notebook
``notebooks/_build_gga_training_dfs_subsets_notebook.py`` into a generic ``cluster/`` package.
...
# ``notebooks/_build_gga_training_dfs_subsets_notebook.py`` (~lines 557-581). The only change is
...
def _bh76w411_pool_builder(cfg):
    ...
    from xcquinox.pipeline.training_points import build_bh76w411_pool_points
