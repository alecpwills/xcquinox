"""xcquinox.pipeline.cluster._preflight: the SLURM preflight-job entrypoint.
...
``preflight.sbatch`` invokes it as::
    ...
    python -m xcquinox.pipeline.cluster._preflight ${RUN_DIR}
...
read-only. Its job is, on a compute node, before the train array starts:
  ...
  3. Build one :class:`~xcquinox.pipeline.config.TrainingSpec` per grid cell via
     :func:`xcquinox.pipeline.cluster.spec_builder.build_training_specs`,
...
----------------------------
:class:`~xcquinox.pipeline.cluster.grid_config.GridConfig` carries an
...
from xcquinox.pipeline.cluster.inputs import prepare_inputs as _inputs_prepare_inputs
...
def _select_smoke_cell(specs):
    ...
    the architectures whose parent offsets were measured as the
    largest, so they are exactly the cells the gate
