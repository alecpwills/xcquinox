"""xcquinox.pipeline.cluster.__main__: CLI dispatch for the HPC training harness.

Invoked as ``python -m xcquinox.pipeline.cluster <subcommand> ...``. This module is
...
lock, and wires the already-built ``cluster/`` modules together. Subcommands:
  ...
  - ``list-runs``: discover ``run_<UTC>Z`` dirs under
    ...
    ``xcquinox/pipeline/cluster/sync.py`` for both.
...
def _resolve_train_scripts(run_dir: str, cfg=None) -> tuple[str, str | None]:
    ...
    for: ``cmd_resubmit`` hard-coded the array pair, so an
...
def cmd_prepare(args) -> int:
    ...
    if recompute_refs and _on_login_node():
        _log(
            ...
            "  - Use `python -m xcquinox.pipeline.cluster submit <grid>`: its "
...
def cmd_submit_eval(args) -> int:
    ...
    """
    from xcquinox.pipeline.cluster._submit_eval import submit_deferred_eval
    try:
...
def cmd_repair_manifest(args) -> int:
    ...
    try:
        ...
        from xcquinox.pipeline.cluster.materialize import _spec_filename
...
def cmd_pull(args) -> int:
    ...
    printed so the caller can pipe it to e.g. ``python -m xcquinox.pipeline.cluster
    results "$(...)"``.
    """
    host = args.host
    remote_root = args.remote_root.rstrip("/")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # The harness is reached two ways: as the console script and as the module. The usage
    # line names the one the caller used.
    invoked = os.path.basename(sys.argv[0]) if sys.argv else ""
    parser = argparse.ArgumentParser(
        prog=invoked if invoked == "xcquinox-cluster" else "python -m xcquinox.pipeline.cluster",
        description="HPC (SLURM) training-harness CLI for xcquinox.pipeline.",
    )
