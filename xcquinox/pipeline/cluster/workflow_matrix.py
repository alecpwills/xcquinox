...
The 2026-08-21 record requires that, before any campaign YAML
...
from xcquinox.pipeline.parallel import pyscf_pool_threads
...
def repo_root_path() -> Path:
    ...
    ``<root>/xcquinox/pipeline/cluster/workflow_matrix.py`` -> ``<root>``.
    """
...
def write_matrix_yaml(arch, out_dir, *, repo_root,
    ...
    if external_refs_dir is None:
        ...
        # The supplied directory is the caller's input and is checked first, so
        # a mistyped path is named even on a checkout that does not carry the
        # untracked cache; the ledger check follows, as before.
...
#: Its architecture-carrying oracles
...
ORACLE_TEST_TARGET = f"xcquinox/pipeline/tests/{ORACLE_MODULE}.py"
...
#: the pool rather than a choice, stated here as the loaded pool gives it. The eval stage
...
#: the two indices are what the template's grid expands to.
_SPEC_INDICES = (0, 1)
...
class Stage:
    ...
    #: sequence: the matrix records its verdict, it does not require a PASS.
...
def stage_plan(run_dir, *, species_slice=HELDOUT_SPECIES_SLICE,
    ...
    stages = [
        Stage("datagen",
              (py, "-m", "xcquinox.pipeline.cluster._datagen", run_dir)),
        Stage("pretrain",
              (py, "-m", "xcquinox.pipeline.cluster._pretrain", run_dir, "0")),
        Stage("certificate",
              (py, "-m", "xcquinox.pipeline.cluster.fidelity", run_dir, "0"),
              allow_nonzero=True),
        Stage("preflight",
              (py, "-m", "xcquinox.pipeline.cluster._preflight", run_dir)),
    ]
    stages += [
        Stage(f"train[{idx}]",
              (py, "-m", "xcquinox.pipeline.cluster._train_task", run_dir,
               str(idx), "--device", device))
        for idx in _SPEC_INDICES
    ]
    stages += [
        Stage(f"eval[{idx}]",
              (py, "-m", "xcquinox.pipeline.cluster._eval_one_spec", run_dir,
               str(idx)),
              env_extra=slice_env)
        for idx in _SPEC_INDICES
    ]
    stages.append(
        Stage("validate_run",
              (py, "-m", "xcquinox.pipeline.cluster.validate_run", run_dir)))
    return tuple(stages)
...
def _base_env(threads):
    ...
    measured curve is on :data:`xcquinox.pipeline.parallel.PYSCF_POOL_THREADS_MAX`).
...
def _required_data_files(grid_path, arch):
    ...
    from xcquinox.pipeline.cluster._datagen import _required_data_specs
    try:
...
def _artefact_paths(run_dir, arch, data_dir, data_specs=None):
    ...
    ``pretrain_data`` label.
    """
    from xcquinox.pipeline.pretrain_data_gen import pretrain_data_filename
...
def _oracle_failure_note(rc, module_path, selector):
    ...
    The oracle module is the program's and this runner does not own it. Absent,
    ...
    if not Path(module_path).is_file():
        return (f"no oracle module: {ORACLE_MODULE}.py is not installed",
                ...
                f"installed at {module_path} (the "
                f"2026-08-21 record), so the selector {selector!r} can match nothing "
...
def _o3_branch_line(log_path):
    ...
    try:
        ...
        return None
    hits = [ln.strip() for ln in lines if ln.startswith("[O3] ")]
...
def run_arch(arch, work_root, *, runner=subprocess.run,
    ...
    stages = [_run_stage(
        "submit",
        (sys.executable, "-m", "xcquinox.pipeline.cluster", "submit",
...
def _is_clean(result) -> bool:
    """True iff this architecture met every acceptance item of the matrix.
    ...
    read here: the matrix records the verdict, it does not require a PASS from a
    ...
    # sequence that is not the one the matrix fixes.
...
def _teardown_abort_note(rc) -> str:
    ...
    return (f" Return code {rc} is a process ABORT (SIGABRT/SIGSEGV), the "
            ...
            "through xcquinox/pipeline/cluster/_exit.py so that a completed "
...
def write_matrix_report(results, path) -> Path:
    ...
    The markdown table is the baseline record (columns: architecture,
...
def main(argv=None, *, runner=subprocess.run) -> int:
    ...
    it -- the matrix records the verdict, it does not require a PASS from a
    50-step pretrain on two atoms --
    ...
    parser = argparse.ArgumentParser(
        prog="python -m xcquinox.pipeline.cluster.workflow_matrix",
