"""xcquinox.pipeline.cluster._pretrain -- per-architecture pretrain-array-task worker.
...
pretrain-array sbatch template invokes this once per array task as::
    ...
    python -m xcquinox.pipeline.cluster._pretrain <RUN_DIR> <ARCH_IDX>
...
For one architecture index it:
  ...
  - Derives the list of distinct architectures from the ``arch`` sweep axis.
    ...
    index fails fast.
  - Builds a :class:`~xcquinox.pipeline.config.PretrainSpec` for that architecture,
...
JAX routing
-----------
Like :mod:`xcquinox.pipeline.cluster._train_task`'s ``--device auto`` default,
...
def _run_pretrain(spec, progress_callback=None):
    """Run :func:`xcquinox.pipeline.pretrain.run_pretrain` -- the test seam.
    ...
    ``spec.checkpoint_dir``.
    """
    from xcquinox.pipeline.pretrain import run_pretrain  # noqa: E402 -- after JAX routing
...
def main(argv=None) -> int:
    ...
    # jax-free; the xcquinox.pipeline.pretrain import in _run_pretrain transitively
    ...
    try:
        ...
        sys.stdout.write(
            ...
            f"is not a known architecture in xcquinox.pipeline.config.ARCHITECTURES\n"
        )
        sys.stdout.flush()
        return 1
