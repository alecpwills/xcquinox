"""xcquinox.pipeline.cluster.fidelity -- the per-architecture physics certificate.
...
Invocation on a node::
    ...
    python -m xcquinox.pipeline.cluster.fidelity <RUN_DIR> <ARCH_IDX>
...
Every jax / equinox / pyscf / ``xcquinox.pipeline.data`` import happens INSIDE a
...
# the v5 meta-GGA-against-SCAN note of 2026-08, Section 5). The bound below is
...
def certificate_describes_run(cfg, pretrain_dir: str, arch_name: str,
    ...
    def _no_identity(v) -> bool:
        # the version fallback ('1+unknown') carries no code identity: on
    ...
    if _no_identity(recorded_version) or _no_identity(running):
        print(f"[fidelity] version cross-check skipped: certificate "
              f"{recorded_version!r} / running {running!r} -- a version "
