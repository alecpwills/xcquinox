"""xcquinox.pipeline.cluster.job_tracking: SLURM job submission log + outcome reduction.
...
This module owns three things:
  ...
  - ``jobs.json``: an append-only submission log living at
    ``<run_dir>/jobs.json``. Append-only because the log is the record of
...
def mark_superseded(run_dir: str, kind: str, generation: int) -> None:
    ...
    record stays in the append-only log (the record) but ``superseded=True``
