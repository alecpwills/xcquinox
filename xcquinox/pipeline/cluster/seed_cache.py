...
Usage::
    ...
    python -m xcquinox.pipeline.cluster.seed_cache <grid_config.yaml> \
...
def _run_scf_with_cache(entry, atoms, **kw):
    from xcquinox.pipeline.external_refs import run_scf_with_cache
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    from xcquinox.pipeline.cluster.spec_builder import atoms_to_pyscf_str
    from xcquinox.pipeline.data import seed_geometry_tag
    n_ok, failed = 0, []
    t0 = time.time()
