"""xcquinox.pipeline.cluster._train_task: per-SLURM-array-task training wrapper.

The train-array sbatch template invokes this once per array task as::

    python -m xcquinox.pipeline.cluster._train_task <RUN_DIR> <SLURM_ARRAY_TASK_ID>

It is the thin harness layer between SLURM and the existing per-spec worker
``xcquinox.pipeline._train_one_spec``. Responsibilities:
...
# verbatim from notebooks/_build_gga_training_dfs_subsets_notebook.py (_GPU_OOM_MARKERS). Kept
...
def main(argv=None) -> int:
    ...
    # to 56 kcal/mol in atomization energies, larger than every effect the
    # training is meant to measure.
