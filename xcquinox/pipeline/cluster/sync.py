"""xcquinox.pipeline.cluster.sync: pure helpers for the ``pull`` subcommand.

The ``pull`` subcommand of ``python -m xcquinox.pipeline.cluster`` rsyncs a sweep
...
def filter_file_path(profile: str) -> Path:
    ...
    pkg = resources.files("xcquinox.pipeline.cluster")
    ...
    if run_id == "latest":
        ...
        if not candidates:
            raise ValueError(
                ...
                "Try `python -m xcquinox.pipeline.cluster list-runs` to "
