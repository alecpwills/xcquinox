"""xcquinox.pipeline.cluster._exit: the exit every harness entry point leaves through.
...
``xcquinox/pipeline/tests/conftest.py``, which is the one remaining copy of it --
...
harness WORKER: ``python -m xcquinox.pipeline.cluster._pretrain`` ran both
