"""xcquinox.pipeline.cluster.grid_config: config layer for the HPC training harness.
...
# 122 here.
from xcquinox.pipeline.orientation_lock_default import (
...
class HyperParams:
    ...
    # PretrainSpec / TrainingSpec convention in xcquinox.pipeline.config.
    ...
    # xcquinox.pipeline.train._run_per_molecule_loop / TrainingSpec.update_scheme.
    ...
    # Results-neutral; see xcquinox.pipeline.padding. Off => byte-identical training.
...
class ModelConfig:
    ...
    by construction. Applied wherever the run
...
class PretrainConfig:
    ...
    stage builds a :class:`xcquinox.pipeline.config.PretrainSpec` per architecture
    ...
    (see ``notebooks/_build_gga_training_dfs_subsets_notebook.py`` and
    ``_build_gga_training_anchor_transfer_notebook.py``):
...
# ``xcquinox.pipeline.config`` pulls JAX and equinox and the harness parser runs on
...
def load_grid_config(path: str) -> GridConfig:
    ...
    ``tools/oep_per_species_tune.py``). JSON uses the stdlib.
...
def validate_grid_semantics(cfg: GridConfig, domain) -> None:
    ...
    for a in cfg.sweep.arch:
        ...
        if a not in ARCHITECTURES:
            # a shown name (xcquinox.pipeline.arch_names) resolves in the registry
            ...
            from xcquinox.pipeline.arch_names import stored_key
            raise ValueError(
