...
def compute_rung35_multishell_occupancy(proj_ao_stack: jnp.ndarray,
    ...
    dm = jnp.asarray(dm)
    # One single-width contraction per width, concatenated alpha-major:
    # columns 2i and 2i + 1 are compute_rung35_occupancy(A[i], dm) bit for
    # bit, so the single-width descriptor is the len(alphas) == 1 member
    # under any XLA. A batched einsum over the width axis is the same sum in
    # a different order (one ulp on 336 of 39232 entries of the H2 probe
    # under jax 0.10.2, none under 0.7.0).
    return jnp.concatenate(
        [compute_rung35_occupancy(A[i], dm) for i in range(A.shape[0])],
        axis=1)                                          # (N, 2 * n_alpha)
