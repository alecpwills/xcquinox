"""xcquinox.pipeline.data: MoleculeData TypedDict and precompute.
...
_ALLOWED_EXTERNAL_KEYS = frozenset({
    ...
    "basis_used",
    # Benchmark density-only refs (xcquinox.pipeline.benchmark_refs) also carry
    ...
    # loader. A key absent from this set makes every held-out
    # evaluation refuse every reference file and
    # write tables of NaN.
    "t1_diagnostic",
...
class MoleculeData(TypedDict, total=True):
    ...
    # rho_init = (1 - beta) rho_DFT + beta rho_atomic, train._mix_seed_batch); None unless the
...
class ExternalDataSchemaError(ValueError):
    ...
    one and writing tables of NaN. A ``ValueError`` still, for the
...
def seed_cache_file(mol_spec: MoleculeSpec, *, seed_cache_dir: str,
    ...
    """
    from xcquinox.pipeline.external_refs import _intermediate_cache_name
...
def precompute_fixed_density_data(
    ...
    XC quadrature, pinned by :mod:`xcquinox.pipeline.pyscf_determinism` so the
    ...
    size by the same module; ``reference_small_rho_cutoff`` records the
    density threshold the reference SCF's grid was pruned with at its first
    cycle, held by the same module at the value pyscf's class default had
    through release 2.11 (1e-7; 0 from 2.14).
    ...
    # system size. The density threshold the grid is pruned with at the
    # first cycle is a release default (1e-7 through pyscf 2.11, 0 from
    # 2.14) and is held at the campaigns' value the same way. The thread
    # count is the caller's; all four go into mol_metadata
    # (reference_xc_blksize, reference_blas_threads, reference_eri_path,
    # reference_small_rho_cutoff) and into no cache identity (see
    # pyscf_determinism).
    ...
    if is_unrestricted:
        ...
        if rung35ms_features is not None:
            from xcquinox.pipeline.rung35 import compute_rung35_multishell_occupancy
    ...
    if "cderi" in all_needed:
        from xcquinox.pipeline.df_jk import build_cderi
    ...
    result = MoleculeData(
        ...
        mol_metadata={
            ...
            # And the density threshold its grid was pruned with at the
            # first cycle: pyscf's class default moved from 1e-7 to 0 in
            # release 2.14 and is held at 1e-7.
            "reference_small_rho_cutoff": reference_pins.small_rho_cutoff,
        },
