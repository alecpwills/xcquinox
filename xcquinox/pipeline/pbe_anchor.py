"""xcquinox.pipeline.pbe_anchor: UKS-aware PBE-anchor regularization.
...
def _pbe_fx_libxc(rho_alpha: jnp.ndarray,
    ...
    ``xcquinox.pipeline.oneshot._uks_spin_resolved_vxc``) and the anchor
    ...
    def _fx_rks(rho_spin_doubled: np.ndarray,
        ...
        # rho-independent at fixed s_sigma, so the branch is continuous.
        return np.where(
