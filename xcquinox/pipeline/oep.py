"""xcquinox.pipeline.oep: Wu-Yang OEP inversion for reference V_xc generation.
...
class OEPResult(NamedTuple):
    ...
    n_iter: int
    density_error: float
    # Provenance for downstream loaders and checks.
...
class _OEPPlateau(Exception):
    ...
    in tools/oep_per_species_emit_overrides.py). The detector watches
...
def _ks_from_vxc_matrix_uhf(mol, mf, vxc_matrix, *, dm0=None, level_shift=0.0,
    ...
    ``xcquinox/pipeline/tests/test_oep_uks.py`` module docstring for
