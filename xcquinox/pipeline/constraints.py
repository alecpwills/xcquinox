"""xcquinox.pipeline.constraints: Constraint ABC, registry, and 4 concrete constraints.
...
class LiebOxfordBound(Constraint):
    ...
    Bounds and why:
      ...
      * Lower bound is 0, the physical floor: the exchange energy density
        ...
        ``F_x ≥ 0`` is the only exact lower constraint, there is no
        ...
        in-network ``_AlecLOB`` squash (``xcquinox/pipeline/networks.py``).
