"""xcquinox.pipeline.parallel -- WorkerJob, WorkerResult, run_workers.
...
def worker_script_path(name: str) -> str:
    """Return the absolute path to a pipeline worker script, for use as
    ...
    Accepts 'pretrain_worker', 'train_worker', 'evaluation_worker' and 'eval_holdout_worker'.
    """
    spec = importlib.util.find_spec(f"xcquinox.pipeline.workers.{name}")
    if spec is None or spec.origin is None:
        raise FileNotFoundError(
            f"could not locate xcquinox.pipeline.workers.{name}"
        )
    return spec.origin
