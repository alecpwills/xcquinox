"""xcquinox.pipeline.solver: SolverConfig, ABCs, and SCF dispatcher.

Implements the SCF solver toggle design:
the SCF-solver and reference-density rename design.

This module intentionally contains no backend SCF logic. Backends are
lazily imported inside `run_scf` so that `import xcquinox.pipeline.solver`
...
def run_scf(config: SolverConfig, model, mol_data: dict,
    ...
    if config.backend == SolverBackend.MANUAL:
        from xcquinox.pipeline.solver_manual import run_manual_scf
    ...
    if config.backend == SolverBackend.PYSCFAD:
        ...
        from xcquinox.pipeline.solver_pyscfad import run_pyscfad_scf
