"""xcquinox.pipeline.workers.eval_holdout_worker: held-out eval shard worker.
...
orchestrator merges. Mirrors ``evaluation_worker.py``: thread env is pinned from
...
def main(args=None):
    parser = argparse.ArgumentParser(description="the held-out evaluation worker")
    ...
    try:
        ...
        from xcquinox.pipeline.workers import _cpu_bind
...
if __name__ == "__main__":
    ...
    # would pull xcquinox.pipeline.cluster -- and JAX with it -- before those caps
    ...
    _exit_spec = _importlib_util.spec_from_file_location(
        "_xcquinox_pipeline_hard_exit", _exit_path)
