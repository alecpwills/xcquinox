"""xcquinox.pipeline.models: AlecGGAModel composite model.
...
# Fx, Fc are masked to 1; this is a numerically-immaterial mask, not an exact
...
def _pack_row(rho_scalar, sigma_scalar, features_row):
    """Assemble a single-row feature vector ``[rho, sigma, *extras]`` for the network."""
...
def _batched_network_apply_polarized(net, rho, sigma, zeta, features):
    """Batched apply with a per-point spin-polarization zeta.
...
class AlecGGAModel(eqx.Module):
    ...
    # Physical constraints are enforced INTRINSICALLY by the networks
    ...
    # expose the network-held constraints under the model-level API.
    @property
    ...
    def eval_Fx(self, rho, sigma, features):
        ...
        simply the batched network apply."""
    ...
    def _ec_baseline(self, rho_safe, zeta):
        ...
        PW92. At zeta=0 the polarized baseline reduces EXACTLY to the
    ...
    def _exc_pieces(self, rho, sigma, features, zeta=0.0):
        ...
        tail masking and rho_safe prefactor match ``eval_exc``, so
        ``ex_density + ec_density == eval_exc`` holds pointwise
        (see test_eval_exc_equals_eval_ex_plus_eval_ec_batched).
        ...
        ``zeta`` is the spin polarization (rho_a - rho_b)/rho_tot. When
        ...
        baseline is used; at zeta=0 the polarized baseline also reduces exactly
        to the unpolarized one.
        """
    ...
    def _exc_pieces_scalar(self, rho_scalar, sigma_scalar, features_scalar, zeta=0.0):
        ...
        ``zeta`` is the spin polarization at this point. A
        ...
        if polarized:
            # zeta at index 2, descriptor extras follow at index 3.
    ...
    def eval_exc_scalar(self, rho_scalar, sigma_scalar, features_scalar, zeta=0.0):
        ...
        Applies the same constraint chain as eval_Fx/eval_Fc. Network inputs
        use raw rho (no pre-clip via rho_cutoff).
        ...
        ``zeta`` forwards to the correlation piece (ignored by a
    ...
    def eval_ec_scalar(self, rho_scalar, sigma_scalar, features_scalar, zeta=0.0):
        ...
        with ``part="c"`` to build the correlation V_xc. With a
    ...
    def constraint_report(self, rho, sigma, features) -> dict:
        ...
        def _c_raw(r, s, f):
            # zeta=0: constraint_report uses the unpolarized packing.
            return jax.vmap(
