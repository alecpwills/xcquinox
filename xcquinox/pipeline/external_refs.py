...
(_build_gga_training_anchor_transfer_notebook.py, the H2O and C2H2 data cells). 2-tier: svp-jkfit primary
...
def build_species_union() -> list[SpeciesEntry]:
    ...
    runs.
    """
    from xcquinox.pipeline import dfs_pool, eval_probes
    from xcquinox.pipeline.subset_selection import _make_hb_atoms, _make_pt_atoms
    ...
    from xcquinox.pipeline.dfs_pool import ATOMIC_GROUND_STATE_SPIN
...
def resolve_geometry(spec: SpeciesEntry):
    ...
    from xcquinox.pipeline.dfs_pool import _g297_traj_path
    from xcquinox.pipeline.subset_selection import _make_hb_atoms, _make_pt_atoms
...
def run_scf_with_cache(
    ...
    *,
    ...
    :mod:`xcquinox.pipeline.pyscf_determinism`) and reference_blas_threads
    ...
    result = {
        ...
        # grid points per block of the XC quadrature, pyscf's OpenMP
        # worker count the SCF ran at, the integral path and the density
        # threshold the grid was pruned with at the first cycle.
...
def run_ccsd_with_cache(
    ...
    *,
    ...
    xcquinox/pipeline/data.py:296-299 for the canonical spin-summing
...
#
# RKS (closed-shell): mirrors the H2O and C2H2 data cells of
# _build_gga_training_anchor_transfer_notebook.py. conv_tol=2e-3 is tuned against the achievable floor for
...
# xcquinox/pipeline/tests/test_oep_uks.py (which accepts density_error < 0.1
...
# tools/oep_per_species_emit_overrides.py after the harness sweep.
...
_PER_SPECIES_OEP_OVERRIDES: dict[tuple[str, int, int], tuple[dict, ...]] = {
    # -- AUTO-GENERATED by tools/oep_per_species_emit_overrides.py
    # -- Source: data/oep_tune/2026-05-06/summary.json
    # -- TODO(citations, REQUIRED before paper write-up): the [oep-tdl-1..6]
    #    keys resolve to TO-DOWNLOAD entries in docs/references.bib
    ...
    # conv_tol mechanism; re-run tools/oep_per_species_emit_overrides.py for a
    ...
    # Tune log: data/oep_tune/2026-05-06/Be.jsonl trial_idx=1
    ("Be", 0, 0): (
        {
    ...
    # Tune log: data/oep_tune/2026-05-06/C+.jsonl trial_idx=0
    ("C+", 1, 1): (
        {
    ...
    # Tune log: data/oep_tune/2026-05-06/F2.jsonl trial_idx=0
    ("F2", 0, 0): (
        {
            "aux_basis": "def2-svp-jkfit",
            "grid_level": 1,
    ...
    # Tune log: data/oep_tune/2026-05-06/F2O.jsonl trial_idx=0
    ("F2O", 0, 0): (
        {
            "aux_basis": "def2-tzvp-jkfit",
            "grid_level": 1,
    ...
    # Tune log: data/oep_tune/2026-05-06/HF.jsonl trial_idx=0
    ("HF", 0, 0): (
        {
            "aux_basis": "def2-svp-jkfit",
            "regularization": 0.0001,
    ...
    # Tune log: data/oep_tune/2026-05-06/HS.jsonl trial_idx=0
    ("HS", 0, 1): (
        {
            "aux_basis": "def2-tzvp-jkfit",
            "inner_damp": 0.1,
    ...
    # Tune log: data/oep_tune/2026-05-06/N2O.jsonl trial_idx=0
    ("N2O", 0, 0): (
        {
            "aux_basis": "def2-tzvp-jkfit",
            "grid_level": 1,
    ...
    # Tune log: data/oep_tune/2026-05-06/O3.jsonl trial_idx=0
    ("O3", 0, 0): (
        {
            "aux_basis": "def2-tzvp-jkfit",
            "grid_level": 1,
...
def run_oep_cascade(
    ...
    *,
    ...
    (xcquinox/pipeline/oep.py:696-700).
    ...
    (e.g. ``tools/smoke_preflight_uks_oep.py``) can show per-tier +
    ...
    # See xcquinox/pipeline/tests/test_oep_uks.py module docstring for
    ...
    for tier_idx, tier in enumerate(tiers):
        ...
        try:
            oep_result = pipeline_oep.run_oep_inversion(
                tier_mol_spec,
                ccsd_payload["dm_ao"],
                aux_basis=aux_basis,
