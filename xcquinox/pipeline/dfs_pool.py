...
#       `xcquinox/pipeline/data/w411_full_pool.json` and in the GMTKN55
#       checkout's `W4-11/.res` (data/gmtkn55/PROVENANCE.md).
...
#       The paper is cited by its DOI above; the table it was parsed
#       into is the package data file (kJ/mol; column E)
#       xcquinox/data/haunschild_g2/g2_97.csv, beside the trajectory
#       the pool geometries are read from.
#
...
DFS_AE_DATA = [
    ...
    {"hill": "C2H2", "name": "Acetylene",
     ...
     "spin": 0, "charge": 0,
     "spin_source": "X¹Σg+ closed-shell (NIST CCCBDB)",
     "source": "W4-11 (Karton 2011, CPL 510, 165; = DFS ref [29]); value in xcquinox/pipeline/data/w411_full_pool.json"},
...
# tracked ``xcquinox/pipeline/data/bh76_full_pool.json`` reaction rows (pinned by
...
# energies* (approved 2026-05-24). Each entry below
...
# 354c3ded: see data/gmtkn55/PROVENANCE.md), the source
...
# ``xcquinox/pipeline/data/bh76_full_pool.json`` species block (n2ohts / RKT11 / hf2ts,
...
# xcquinox/pipeline/losses.py::L5GradnormVxcStep7._ip13_channel which reads
...
def _g297_traj_path() -> Path:
    """The G2/97 ASE trajectory: package data under xcquinox/data/haunschild_g2."""
    return Path(__file__).resolve().parents[1] / "data" / "haunschild_g2" / \
        "g2_97.traj"
