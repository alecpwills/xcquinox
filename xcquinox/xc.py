...
from .utils import lda_x, pw92c_unpolarized, pw92c_unpolarized_scalar
...
class RXCModel_GGA(eqx.Module):
    ...
    Handles batched input with internal vmap.

    Input format: (N, 2) for unpolarized or (N, 5) for polarized.
    - Unpolarized: each row is [rho, sigma]
    - Polarized: each row is [rho_a, rho_b, sigma_aa, sigma_ab, sigma_bb]

    Output format: (N,) epsilon values.

    The networks expect [rho, sigma] for one grid point, so vmap is used internally.
    For polarized input, total rho and sigma are computed before passing to networks.
    '''
    xnet: eqx.Module
    cnet: eqx.Module
    ...
    def _eval_single_point(self, point):
        '''
        Evaluate epsilon for a single grid point (2 features: rho, sigma).

        :param point: Array of shape (2,) with [rho, sigma]
        ...
        ex_lda = lda_x(rho)
        ec_pw92 = pw92c_unpolarized_scalar(rho)
    ...
    def __call__(self, inputs):
        '''
        Compute epsilon = rho * e_xc for batched grid points.

        :param inputs: Shape (N, 2) for unpolarized or (N, 5) for polarized.
        :type inputs: jax.Array
        :return: epsilon array of shape (N,) or scalar for single point
        :rtype: jax.Array
        '''
        ...
        if n_features == 2:
            # Unpolarized: [rho, sigma] per point
            epsilon = jax.vmap(self._eval_single_point)(inputs)
        elif n_features == 5:
            # Polarized: [rho_a, rho_b, sigma_aa, sigma_ab, sigma_bb]
            # Convert to [rho_total, sigma_total] for the networks
            rho_a = inputs[:, 0]
            rho_b = inputs[:, 1]
            sigma_aa = inputs[:, 2]
            sigma_ab = inputs[:, 3]
            ...
            # Compute LDA components with spin polarization
            ex_lda = jax.vmap(lda_x)(rho)
            ec_pw92 = jax.vmap(lda_c_pw)(rho_a, rho_b)

            # Networks get [rho, sigma]
            net_inputs = jnp.stack([rho, sigma], axis=1)
            Fx = jax.vmap(self.xnet)(net_inputs).squeeze()
            Fc = jax.vmap(self.cnet)(net_inputs).squeeze()
...
class RXCModel_GGA_Extended(eqx.Module):
    '''
    Extended GGA XC model supporting additional features (cusp, DM features).

    This model extends RXCModel_GGA to handle networks that accept more than
    ...
    routes inputs appropriately to the extended networks.
    '''
    xnet: eqx.Module
    cnet: eqx.Module
    use_cusp: bool
    ...
    def _get_expected_features(self):
        '''Calculate expected number of input features based on config.'''
        n = 2  # rho, sigma
        if self.use_cusp:
            n += 2  # cusp_factor, log_weighted_Z
        if self.use_dm_features:
            n += self.n_dm_features
        return n

    def _eval_single_point(self, point):
        '''
        Evaluate epsilon for a single grid point with extended features.

        :param point: Array with [rho, sigma, ...optional features...]
        :return: Scalar epsilon value
        '''
        rho = point[0]
        sigma = point[1]
        ...
        rho_safe = jnp.maximum(rho, 1e-18)
        # Networks receive the full point including extended features
        Fx = jnp.sum(self.xnet(point))
        Fc = jnp.sum(self.cnet(point))
        epsilon = rho_safe * (ex_lda * Fx + ec_pw92 * Fc)
        return epsilon
    ...
    def __call__(self, inputs):
        '''
        Compute epsilon = rho * e_xc for batched grid points with extended features.

        :param inputs: Shape (N, F) where F depends on enabled features:
                       - Basic: F=2 [rho, sigma]
                       - Cusp: F=4 [rho, sigma, cusp_0, cusp_1]
                       - DM: F=5 [rho, sigma, dm_0, dm_1, dm_2]
                       - Combined: F=7 [rho, sigma, cusp_0, cusp_1, dm_0, dm_1, dm_2]
        ...
        if n_features != expected:
            raise ValueError(
                f"Input shape mismatch: got {n_features} features, "
                f"expected {expected} (use_cusp={self.use_cusp}, "
                f"use_dm_features={self.use_dm_features})"
            )

        # Use vmap to process all grid points
        epsilon = jax.vmap(self._eval_single_point)(inputs)
        return jnp.squeeze(epsilon)
