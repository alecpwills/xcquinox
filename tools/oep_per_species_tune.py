...
Usage:
  python tools/oep_per_species_tune.py \\
      --species Be "C+" F2 F2O HF HS N2O O3 \\
      --grid tools/oep_tune_grids.yaml \\
...
class _Heartbeat:
    ...
    tools/smoke_preflight_uks_oep.py:73-110. Daemon thread prints a
...
def _is_atomic_species(name: str) -> bool:
    ...
    xcquinox/pipeline/external_refs.py:resolve_geometry."""
...
def main() -> int:
    ...
    parser.add_argument("--grid", type=Path,
                        default=Path("tools/oep_tune_grids.yaml"))
    ...
    # Allowlist mirrors xcquinox/pipeline/external_refs.py:_OVERRIDE_TIER_KNOB_ALLOWLIST
