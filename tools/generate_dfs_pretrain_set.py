"""Export the DFS pretraining set to xcquinox/pipeline/data/dfs_pretrain_set.json.
...
The trajectory is package data (xcquinox/data/haunschild_g2/g2_97.traj, the
file dfs_pool reads); the geometries are exported ONCE into a JSON beside it
so that the certificate and the pretraining data generator resolve the same
geometries with no ASE dependency at run time.

Usage:
    python tools/generate_dfs_pretrain_set.py [--traj PATH] [--out PATH]
"""
import argparse
import json
import os
...
# the package's copy of the trajectory, the same file the training pool is built from
DEFAULT_TRAJ = str(_g297_traj_path())
...
def _check_name_matches_geometry(record):
    ...
    """
    from xcquinox.pipeline.dfs_pretrain_set import formula_from_name
...
def main(argv=None):
    ...
    if out is None:
        ...
        out = os.path.join(os.path.dirname(os.path.abspath(
            xcquinox.pipeline.__file__)), "data", "dfs_pretrain_set.json")
