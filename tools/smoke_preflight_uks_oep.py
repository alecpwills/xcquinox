#!/usr/bin/env python
"""Smoke verification for xcquinox.pipeline.external_refs.preflight_uks_oep.
...
Usage:
    python tools/smoke_preflight_uks_oep.py --cache-dir /tmp/smoke
    python tools/smoke_preflight_uks_oep.py --cache-dir /tmp/smoke --dry-run
...
def dump_npz(npz_path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    """
    # Safe-load contract: verbatim from xcquinox/pipeline/external_refs.py:215
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        from xcquinox.pipeline.external_refs import preflight_uks_oep
