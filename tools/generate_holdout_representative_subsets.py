...
meta-GGA alpha coordinate the GGA can't see):
  tools/analysis/bh76w411_representative_subsets/<mode>/subset_index_log.json
    { "jsd/<r>": {chosen_indices, metric_value, point_names, point_kinds,
                  held_out_names} }
  + summary.md

Usage:
  python tools/generate_holdout_representative_subsets.py --sizes 1 2 3 4 5 6 7
  python tools/generate_holdout_representative_subsets.py --sizes 1 2 3 --modes alpha_off
"""
import argparse
import json
import math
...
from xcquinox.pipeline.subset_selection_parallel import select_subset_parallel


BASIS = "def2-svp"
GRID_LEVEL = 1
OUT_ROOT = (Path(__file__).resolve().parents[1]
            / "tools" / "analysis" / "bh76w411_representative_subsets")
...
def _select(pool, edges, h_ref, r, weights, args):
    ...
    if args.selector in ("auto", "gpu"):
        try:
            from xcquinox.pipeline.subset_selection_gpu import select_subset_gpu
