...
``xcquinox/pipeline/tests/test_cluster_spec_golden.py``.

What the snapshot is for
------------------------
The cluster harness (``xcquinox.pipeline.cluster``) is a de-notebooked extraction
of the step-7 spec-building logic in ``notebooks/_build_gga_training_dfs_subsets_notebook.py``.
...
``notebooks/_build_gga_training_dfs_subsets_notebook.py`` (so a stale snapshot -- captured before
the notebook builder changed -- is detectable), and writes it to::

    xcquinox/pipeline/tests/data/notebook_spec_snapshot.json
...
``xcquinox/pipeline/tests/data/notebook_spec_snapshot.json``, and commit it. The
slow test (``pytest -m slow xcquinox/pipeline/tests/test_cluster_spec_golden.py``)
...
``git log -1 --format=%H -- notebooks/_build_gga_training_dfs_subsets_notebook.py``. If you
...
NOTEBOOK_BUILDER = os.path.join(
    REPO_ROOT, "notebooks", "_build_gga_training_dfs_subsets_notebook.py"
)
SNAPSHOT_PATH = os.path.join(
    REPO_ROOT, "xcquinox", "pipeline", "tests", "data",
...
def main(argv=None) -> int:
    ...
    snapshot = {
        ...
        "notebook_builder_path": "notebooks/_build_gga_training_dfs_subsets_notebook.py",
    ...
    print("  git log -1 --format=%H -- notebooks/_build_gga_training_dfs_subsets_notebook.py")
