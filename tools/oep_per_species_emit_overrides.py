...
tools/oep_per_species_tune.py; applies the selection rule (spec sec. 7.1)
to pick a winner per species; emits a paste-ready Python snippet for
xcquinox/pipeline/external_refs.py:_PER_SPECIES_OEP_OVERRIDES.

Usage:
  python tools/oep_per_species_emit_overrides.py \\
      --summary-path <tune-dir>/summary.json \\
      --out-dir      <tune-dir> \\
      [--dry-run]

<tune-dir> is a directory tools/oep_per_species_tune.py wrote; the run behind the
current overrides is kept under data/oep_tune/2026-05-06/.
...
def _emit_snippet(winner: dict, tune_log_path: Path) -> str:
    ...
    return (
        ...
        f'    # Per-species pathology: see tools/oep_tune_grids.yaml\n'
...
def main() -> int:
    ...
    snippets = [
        "# AUTO-GENERATED snippet for xcquinox/pipeline/external_refs.py:_PER_SPECIES_OEP_OVERRIDES.\n"
        "# Source: %s\n" % summary_path
        + "# Citations: [oep-tdl-1..6] in docs/references.bib --\n"
