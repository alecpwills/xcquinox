...
WHY: `xcquinox/pipeline/full_benchmark_pools.py` previously mislabeled the GMTKN55
...
def _real_spec_loader(spec_path: Path):
    from xcquinox.pipeline.eval_holdout import load_training_spec
...
def _real_precompute_fn(training_spec, mol_specs):
    from xcquinox.pipeline.eval_holdout import precompute_holdout_for_spec
...
def _real_eval_fn(training_spec, model, mol_specs, reactions, out_dir, mol_data):
    from xcquinox.pipeline.eval_holdout import run_full_holdout_eval
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--density-refs", default=None,
                   help="dir of density-only benchmark CCSD reference "
                        "<name>.npz files (xcquinox.pipeline.benchmark_refs); "
