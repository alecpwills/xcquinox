...
Usage::
    ...
    python tools/analysis/md_to_tex.py reports/v7/REPORT_v7_2026-09-15.md \\
...
def figure_paths(text: str) -> List[str]:
    ...
    ``\\includegraphics[...]{path}`` lines, in order. A LaTeX argument may quote a path
    that carries spaces; the quotes are the argument's, not the path's, and are dropped."""
    out = []
    for markdown, latex in _IMAGE_REF.findall(text):
        if latex and len(latex) >= 2 and latex[0] == latex[-1] == '"':
            latex = latex[1:-1]
        out.append(markdown or latex)
    return out
