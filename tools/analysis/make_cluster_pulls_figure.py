...
``python -m xcquinox.pipeline.cluster pull <run> --profile full --specs <...>``
...
the harness ``pull`` workflow stages, aggregates:
  ...
  - per-spec ``status`` and ``final_loss`` from
    :func:`xcquinox.pipeline.cluster.analyze.collect_results`.
...
# ---------------------------------------------------------------------------
# House style -- match tools/analysis/make_multimode_figure.py
# ---------------------------------------------------------------------------
...
def load_final_losses(run_dir: Path) -> Dict[int, Optional[float]]:
    ...
    :func:`xcquinox.pipeline.cluster.analyze.collect_results`. Used to drive the
