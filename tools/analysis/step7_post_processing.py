"""Step-7 post-processing: the six subset-sweep figures and headline.json from the eval_df.csv
files of the trained specs (10 sizes x 2 metrics x 2 augmentations x 2 solvers; r=21 excluded
as the full pool), and two more figures with two tables from the held-out probe evaluations
(the per_molecule.json files beside the specs) where they exist; the closing line states the
counts written.

The specs are read from the step-7 checkpoints tree (``--step7-root``, by default
``notebooks/checkpoints_step7`` under the repository) and the outputs are written under
``--out-dir`` (by default ``tools/analysis/figures_step7_post_processing``, an ignored
directory beside this tool). The step-7 notebook runs this tool after its training cells.

Usage, from the repository root::

    python tools/analysis/step7_post_processing.py [--step7-root DIR] [--out-dir DIR]
"""
...
REPO = Path(__file__).resolve().parents[2]  # analysis/ -> tools/ -> repo root
DEFAULT_STEP7_ROOT = REPO / "notebooks" / "checkpoints_step7"
DEFAULT_OUT_DIR = REPO / "tools" / "analysis" / "figures_step7_post_processing"
...
def load_specs(step7_root: Path) -> pd.DataFrame:
    """The eval_df.csv rows of every trained spec under ``step7_root``, tagged with the
    spec's metric, subset size, augmentation and solver; specs absent from the tree are
    skipped, and a tree without any is refused by name."""
    rows = []
        ...
        for r in SUBSET_SIZES:
            for aug in AUGMENTATIONS:
                ...
                for solver in SOLVERS:
                    eval_csv = (Path(step7_root) / metric / tag /
                    ...
                    rows.append(df)
    if not rows:
        raise SystemExit(f"No eval_df.csv files found under {step7_root}; run training first.")
    return pd.concat(rows, ignore_index=True)


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    ap.add_argument("--step7-root", type=Path, default=DEFAULT_STEP7_ROOT,
                    help="the step-7 checkpoints tree the specs are read from")
    ap.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR,
                    help="where the figures, the two tables and headline.json are written")
    return ap.parse_args(argv)


def main(argv: Optional[Sequence[str]] = None) -> int:
    args = parse_args(argv)
    step7_root = Path(args.step7_root)
    out = Path(args.out_dir)
    df = load_specs(step7_root)
    out.mkdir(parents=True, exist_ok=True)
    headline = {}
    written_figures: list = []
    written_tables: list = []

    def _save(fig, path):
        fig.savefig(path, dpi=150)
        plt.close(fig)
        written_figures.append(path)
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    _save(fig, out / "plot1_subset_sweep_ae.png")
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    _save(fig, out / "plot2_subset_sweep_rho.png")
    ...
    fig.tight_layout()
    _save(fig, out / "plot3_overlap_jaccard.png")
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    _save(fig, out / "plot4_metric_vs_r.png")
    ...
    fig.tight_layout()
    _save(fig, out / "plot5_descriptor_overlay.png")
    ...
    if not bar_data.empty:
        ...
        ax.legend()
    fig.tight_layout()
    _save(fig, out / "plot6_w411_transfer.png")
    ...
    def _probe_signed_errors(_df_unused: pd.DataFrame, probe_name: str) -> pd.DataFrame:
        ...
        directly from the per_molecule.json files written by pipeline.run_test.
    ...
    if not df_err.empty:
        ...
        if not mae_rmax.empty:
            ...
            for tick in ax.get_xticklabels():
                ...
                tick.set_ha("right")
            fig.tight_layout()
            _save(fig, out / "plot7_probe_comparison.png")
    ...
    if not df_err.empty:
        ...
        if not best.empty:
            ...
            fig.tight_layout()
            _save(fig, out / "plot8_probe_heatmap.png")
    ...
    if not df_err.empty:
        ...
        out_md = out / "probe_summary.md"
        out_md.write_text("\n".join(md_lines) + "\n")
        written_tables.append(out_md)
    ...
    if not df_err.empty:
        ...
        if not per_mol.empty:
            ...
            try:
                from xcquinox.pipeline import eval_probes
                ref_records: list = []
            ...
            csv_path = out / f"per_molecule_errors_r{R_MAX}.csv"
            best_per_mol[cols].to_csv(csv_path, index=False)
            written_tables.append(csv_path)
    ...
    print(f"Wrote {len(written_figures)} figures, {len(written_tables)} tables and "
          f"headline.json under {out}")
    return 0
