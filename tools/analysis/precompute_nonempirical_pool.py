...
lesson). The PBE density takes the no-SCF fast path from
...
completes -- see hpcjobs/nonempirical_pool.sbatch):
    python tools/analysis/precompute_nonempirical_pool.py \
...
def _load_ref_arrays(ms, refs_dir: Optional[Path], *,
    ...
    if basis is not None and grid_level is not None:
        from xcquinox.pipeline.benchmark_refs import _benchmark_npz_is_complete
...
def run(pool: str, *, basis: str, grid_level: int, out_dir: Path,
    ...
    from xcquinox.pipeline.evaluation import density_eps_terms
