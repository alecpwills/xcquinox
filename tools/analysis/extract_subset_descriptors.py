...
``xcquinox.pipeline.precompute_fixed_density_data``). Total: ~3-5 min for the
current 119 specs.

Usage::

    # Process every pulled category, every spec with a .spec file:
    python tools/analysis/extract_subset_descriptors.py --auto

    # Or a single run dir + specific spec ids:
    python tools/analysis/extract_subset_descriptors.py \\
        <run_dir> --specs 0,1,5
"""
...
def compute_feature_means_for_molecule(mol_spec: Any,
    ...
    try:
        mol_data = pipeline.precompute_fixed_density_data(
