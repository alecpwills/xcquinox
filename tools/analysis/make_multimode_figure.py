...
Usage::
    python tools/analysis/make_multimode_figure.py [RESULTS.json] [OUT_PREFIX]

``RESULTS.json`` defaults to ``demo_logs/multimode_polarized.json`` beside this script, the
file the evaluation writes for the polarized configuration; it is a study output and is not
tracked, so the evaluation runs first, and an absent input exits with that command named.
"""
...
def main(argv=None) -> int:
    ...
    if not os.path.isfile(json_path):
        print(f"{json_path} does not exist: it is the results file that "
              f"multimode_constraint_eval.py writes (its --out; by default "
              f"demo_logs/multimode_<config>.json beside that script), so that runs first",
              file=sys.stderr)
        return 2
