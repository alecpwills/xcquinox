...
error -- the failure diagnosed on 2026-08-03.
...
Usage (example -- run offline; NOT part of the fast figure/test path):
    python tools/analysis/precompute_scan_pool.py \
        --basis def2-svp --grid 2 --pool all \
        --out-dir tools/analysis/scan_cache
...
def _reference_density(refs_dir: Path, name: str):
    ...
    ``grid_weights`` key (``xcquinox.pipeline.benchmark_refs`` writes both)."""
    import numpy as np
...
def main(argv: Optional[list] = None) -> int:
    ...
    p.add_argument("--refs-dir", default=None,
                   help="benchmark density-reference directory (<name>.npz from "
                        "xcquinox.pipeline.benchmark_refs) read by --with-density. "
