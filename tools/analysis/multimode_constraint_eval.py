...
Run:
    python tools/analysis/multimode_constraint_eval.py            # full (16 seeds)
    python tools/analysis/multimode_constraint_eval.py --seeds 2 --species-limit 4  # smoke
"""
...
from xcquinox.pipeline.solver import (  # noqa: E402
...
def _one_energy(pipeline, cfg, model, md) -> float:
...
def species_energies_mode(model, mol_data_by_name: dict, mode: str) -> dict:
    ...
    import xcquinox.pipeline as pipeline  # lazy: pulls pyscf, only needed at compute time
    cfg = solver_config_for_mode(mode)
    return {name: _one_energy(pipeline, cfg, model, md)
...
def _pretrain_one(demo, pipeline, level_spec, weighting, data_dir, ckpt_dir, n_steps,
    ...
    if reuse and have:
        ...
        if should_reuse_checkpoint(cand, n_steps, weighting):
            md = cand
    if md is None:
        md = pipeline.run_pretrain(
            pipeline.PretrainSpec(arch=arch, data_dir=data_dir, checkpoint_dir=ckpt_dir,
...
def main(argv=None) -> int:
    ...
    for i, (name, spec) in enumerate(mol_specs.items(), 1):
        md = pipeline.precompute_fixed_density_data(spec, descriptors=(),
                                                required_keys=("eri",))
        mol_data[name] = md
    ...
    for w in weightings:
        for label, spec in levels:
            ...
            model, conv = _pretrain_one(demo, pipeline, spec, w, data_dir, ckpt,
        ...
        for mode in modes:
            ...
            for name, md in mol_data.items():
                for s, model in enumerate(rand_models):
                    rand_en[s][name] = _one_energy(pipeline, cfg, model, md)
                for w in weightings:
                    pre_en[w][name] = _one_energy(pipeline, cfg, pretrained[(w, label)], md)
                if mode != "fixed_rho":
                    jax.clear_caches()
