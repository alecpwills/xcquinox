#!/usr/bin/env python
"""Demo: physical constraints + pretraining for xcquinox.pipeline XC networks.
...
constraint-aware-pretrained (single seed; library ``pipeline.run_pretrain``).
...
GMTKN55 clone at data/gmtkn55/ (W4-11 subset).

Run::

    python tools/analysis/constraint_pretrain_gmtkn55_demo.py
...
def precompute_all(mol_specs):
    ...
    for i, (name, spec) in enumerate(mol_specs.items(), 1):
        ...
        out[name] = pipeline.precompute_fixed_density_data(spec, descriptors=())
    return out
...
def species_energies(model, mol_data_by_name):
    ...
    for name, md in mol_data_by_name.items():
        out[name] = (float(md["E_pbe"]) if model is None
                     else float(pipeline.fixed_density_total_energy(model, md)))
    return out
...
def pretrain_and_load(spec, data_dir, ckpt_dir):
    ...
    pipeline.run_pretrain(pipeline.PretrainSpec(
