...
#
#     bash tools/analysis/pull_and_plot_v4.sh          # pull + plot
#     bash tools/analysis/pull_and_plot_v4.sh --plot-only
#
...
for arm in $ARMS; do
  if [ -d "$RESULTS_ROOT/$arm/runs" ]; then
    JAX_PLATFORMS=cpu python tools/analysis/make_ablation_arch_figure.py \
...
JAX_PLATFORMS=cpu python tools/analysis/merge_v4_arms.py \
