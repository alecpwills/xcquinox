...
#: (``xcquinox/pipeline/cluster/domain.py``: ``KCAL_PER_HA = 627.5094740631``).
...
#: PretrainSpec's own defaults for those fields (xcquinox/pipeline/config.py), the
...
def cell_spec(arch, data_path, checkpoint_dir, *, weight, n_steps, seed,
    ...
    if cfg is not None:
        from xcquinox.pipeline.cluster._pretrain import pretrain_spec_from_config
...
def main(argv=None):
    ...
    from xcquinox.pipeline.config import list_architectures
    from xcquinox.pipeline.pretrain_data_gen import PRETRAIN_ORIENTATION_LOCK_STRENGTH
...
if __name__ == "__main__":
    ...
    # FAILED (see xcquinox/pipeline/tests/conftest.py) and that reported a
