...
The repo is a git clone at `$GROUP/Alec/xcquinox`, updated with `git pull` after a merge and never by a file copy; `~/xcquinox` is a
...
## 3. Build the parity conda env in the group dir

`$HOME` is too small. The environment lives under `$GROUP` so it is persistent and
visible from every compute node, at the prefix every job script and configuration names:
`$GROUP/Alec/conda_envs/xcquinox_j0102` (the suffix is the jax release, 0.10.2). It is
built by one job on a short milan queue: conda supplies python 3.12, pip installs
`requirements.txt` (the exact releases the workstation runs; every one a PyPI wheel on the
nodes' glibc 2.34, Rocky Linux 9.6, measured 2026-09-22 on `milan1` and on `dn052`) and the
package from the checkout, and the job validates by printing every version, running a
pyscfad SCF and importing the package:

```bash
cd $GROUP/Alec/xcquinox
sbatch hpcjobs/build_parity_env.sbatch
tail -f parity_env_build_<jobid>.log      # ends with "== ENV READY =="
```

The node needs network access (conda and pip fetch). After the checkout is re-cloned, the
one step to repeat inside the environment is `pip install -e $GROUP/Alec/xcquinox --no-deps`.

Verify the install afterwards and record the conda profile path the YAML needs:

```bash
conda activate $GROUP/Alec/conda_envs/xcquinox_j0102
python -c "import xcquinox; print(xcquinox.__file__)"
# expect: /gpfs/projects/FernandezGroup/Alec/xcquinox/xcquinox/__init__.py
python -c "from xcquinox.pipeline import cluster; print('cluster ok')"
python -c "import jax, pyscf, pyscfad, pyscfadlib; print(jax.__version__, pyscf.__version__, pyscfad.__version__, pyscfadlib.__version__)"
# expect: 0.10.2 2.14.0 0.3.4 0.3.4
echo "$(conda info --base)/etc/profile.d/conda.sh"
# save this output -- it goes in cluster.conda_profile in step 5
```

The previous environment, `xcquinox_j070` (jax 0.7.0, pyscf 2.11.0, pyscfad 0.1.11), is the
one the v7 campaigns ran in; the build does not remove it, no job script or configuration
names it any more, and the development history records it beside the v7 results.
...
ls $SCRATCH/
conda activate $GROUP/Alec/conda_envs/xcquinox_j0102
python -c "import xcquinox; from xcquinox.pipeline import cluster; print('ok')"
exit
```
...
| `cluster.conda_env` | `/gpfs/projects/FernandezGroup/Alec/conda_envs/xcquinox_j0102` (the parity environment of section 3; every tracked configuration carries it) |
...
```bash
cd $GROUP/Alec/xcquinox
python tools/capture_notebook_spec_snapshot.py
```
...
- Conda activation lines reference `/gpfs/projects/FernandezGroup/Alec/conda_envs/xcquinox_j0102`.
...
python -m xcquinox.pipeline.cluster resubmit-preflight  "$RUN_DIR" --submit

# rebuild a corrupt/missing manifest.json (non-destructive):
python -m xcquinox.pipeline.cluster repair-manifest     "$RUN_DIR"
```
...
> --suite` command), see [`docs/pipeline/pull_and_figures.md`](../docs/pipeline/pull_and_figures.md).
...
python -m xcquinox.pipeline.cluster list-runs --depth 7
```
...
python -m xcquinox.pipeline.cluster pull latest --category alpha_off/runs

# Same for alpha_on or polarized:
python -m xcquinox.pipeline.cluster pull latest --category alpha_on/runs
python -m xcquinox.pipeline.cluster pull latest --category polarized/alpha_on

# A specific run by stamp:
python -m xcquinox.pipeline.cluster pull run_20260528T143052Z --category alpha_off/runs

# Preview without transferring:
python -m xcquinox.pipeline.cluster pull latest --category alpha_off/runs --dry-run

# When you need the actual trained models for re-evaluation:
python -m xcquinox.pipeline.cluster pull latest --category alpha_off/runs --profile full

# Then analyze locally -- the local tree mirrors the category:
python -m xcquinox.pipeline.cluster results \
...
The filter files at `xcquinox/pipeline/cluster/filters/{summaries,full}.filter`
are exercised on every test run by the canary tests in
`xcquinox/pipeline/tests/test_cluster_sync.py`. Any future change to the
...
1. **Identify which specs are worth re-evaluating** using
   ...
   ```bash
   python tools/analysis/make_cluster_pulls_figure.py
   ```
...
2. **Pull the checkpoints.** With the current `deep_combined_attn` arch
   ...
   ```bash
   python -m xcquinox.pipeline.cluster pull auto --days 0 --profile full --yes
   ```
   ...
   for cat in alpha_on/runs alpha_off/runs polarized/alpha_off/runs; do
     python -m xcquinox.pipeline.cluster pull latest --category "$cat" --profile full
   done
   ```
   ...
   ```bash
   python -m xcquinox.pipeline.cluster pull latest \
...
3. **Run the local re-eval script.** The recommended invocation
   ...
   ```bash
   python tools/analysis/local_reeval.py --auto
   ```
   ...
   ```bash
   nohup python tools/analysis/local_reeval.py --auto \
   ...
   ```bash
   python tools/analysis/local_reeval.py \
   ...
   - `<run_dir>/checkpoints/spec_<NNNN>/eval/local_subset_descriptors.json`
     ...
     by `python tools/analysis/extract_subset_descriptors.py --auto`
...
- Add an `inputs.holdout_molecules_path` field to the grid config schema
  (`xcquinox/pipeline/cluster/grid_config.py:InputPaths`).
...
## 11. Standalone job scripts and the tracked configurations

The harness command line (`python -m xcquinox.pipeline.cluster ...`, sections 7 to 10) renders
and submits the campaign stages; the scripts below are the standalone jobs and tools beside
it, and every one of them is listed here.

| script | what it does | tests that read it |
|---|---|---|
| `pipeline_regression.sbatch` | runs the regression suites of the training-path SCF seam on a compute node | -- |
| `build_parity_env.sbatch` | builds the parity environment on a short milan queue: conda supplies python 3.12, pip installs `requirements.txt` and the package, and the job validates the stack (section 3) | `test_build_parity_env.py`, `test_standalone_sbatch_activation.py` |
| `coldstart_holdout.sbatch` | the cold-start held-out channel over the completed specs of a run (`cluster.coldstart_retro`) | -- |
| `converged_retro.sbatch` | the converged-SCF held-out re-evaluation of completed cells (`docs/pipeline/pull_and_figures.md`) | -- |
| `dfs6311_c2_ref_probe.py` | read-only provenance probe of the benchmark-reference cache for c2 (open item 16) | -- |
| `dfs6311_nan_isolate.py`, `dfs6311_nan_isolate.sbatch` | exact-state replay and decomposition of the training NaN on the BH76 reaction (open item 23) | -- |
| `dfs6311_nan_verify.py`, `dfs6311_nan_verify.sbatch` | production-scale verification of the corrected V_xc on the meta-GGA path (open item 23) | `test_job_digest.py` |
...
`bh76w411_repr.svp_grid2.yaml` and `bh76w411_repr.tzvpd_grid2_df.yaml` (the grid-2 campaigns
the user guide walks through), `dfs_step7.dfs6311_grid3_v3.yaml` (the grid-3 lineage root the
loss primer cites line by line) and the six v7 files: `dfs_step7.dfs6311_grid3_v7g1_size.yaml`,
`dfs_step7.dfs6311_grid3_v7g2a_families_core.yaml` and
...
- **`pretrain.data_dir`**: `/gpfs/projects/FernandezGroup/Alec/xcquinox/notebooks/checkpoints_step6/pretrain_data`
  ...
  (cells 469 and 552 of `gga_training_dfs_subsets.ipynb`), which is what the
  pretrain stage's `run_pretrain` loads via
  `os.path.join(spec.data_dir, "pretrain_data.npz")` (see
  `xcquinox/pipeline/pretrain.py:225`). Present in the checkout after the pull; verify on cluster:
...
- **`cluster.account`**: leave blank. This account has not been needed
  ...
  `xcquinox/pipeline/cluster/grid_config.py:203`; the `_optional_sbatch_line` helper
