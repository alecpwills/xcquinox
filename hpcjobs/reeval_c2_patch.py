...
* ``per_reaction.json`` -- the ``w411_c2_atomization`` row's
  ...
  channel (gated: E_pbe(c) must agree across every surveyed channel).
...
exact agreed value and refuse on ANY disagreement. All gates run before the
...
The survey is only as current as the local pull: refresh first and re-survey
after the training array drains (the banner prints the exact pull
command). ``--dry-run`` stops after the survey. The benchmark CCSD
...
Exit codes
----------
* ``0`` -- survey-only completion (``--dry-run``), nothing patchable, or
...
* ``3`` -- write-phase failure: the channels named as committed were
  ...
  cluster.
* ``4`` -- the survey found channels in an unknown / no-c2 state and
...
#: (80/80 measured), patched from the exact agreed value; any disagreement
...
class PartialWriteError(RuntimeError):
    ...
    no longer holds: ``committed`` lists the :class:`ChannelRecord` entries
...
def survey_run(run_dir, specs=None):
    """One :class:`ChannelRecord` per (spec dir, existing channel dir).

    Every spec dir present under ``<run_dir>/checkpoints`` that has at
    least one eval channel is surveyed; ``specs`` optionally restricts to
...
def _read_output_root(run_dir) -> str | None:
    ...
    read (no config import, so the survey stays light)."""
...
def _pull_refresh_command(run_dir) -> str:
    ...
    return (f"python -m xcquinox.pipeline.cluster pull {Path(run_dir).name} "
            f"--category {cat}")
...
def _recompute_reference(cfg, cell, sc, bench_refs_dir):
    ...
    from xcquinox.pipeline.eval_holdout import descriptors_and_required_keys
    ...
    return pipeline.precompute_fixed_density_data(
        spec,
...
def _nn_record_for_channel(model, md, sc):
    ...
    from xcquinox.pipeline.eval_holdout import (density_errors_for_record,
                                            evaluate_holdout,
...
def _load_channel(record: ChannelRecord) -> ChannelPlan:
    ch = record.path
    what = f"for spec {record.spec} {record.channel}"
    ...
    return ChannelPlan(record=record, pm_rows=pm_rows, pr_rows=pr_rows,
...
def _plan_patch(plan: ChannelPlan, fresh: dict, pool: dict) -> None:
    ...
    :func:`_collect_agreement`. The SCF-dependent model-free fields are
    written from the LOCAL RECOMPUTE (already band-gated against the
    clean envelope); the grid pair is written from the exact agreed value.
    ...
    for k in MODEL_FREE_FIELDS:
        ...
        if v is None and _finite(old.get(k)):
            raise PatchRefused(
                f"spec {plan.record.spec} {plan.record.channel}: no clean-"
...
def _collect_agreement(run_dir, survey_rows):
...
def _push_sheet(run_dir, patched) -> str:
    ...
    for record in patched:
        rel = f"spec_{record.spec:04d}/{record.channel}"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--specs", default=None,
                        help="Optional comma-separated spec indices to "
                             "restrict the survey and patch to.")
    parser.add_argument("--dry-run", action="store_true",
                        help="Survey and report only; write nothing.")
    ...
    _print("=" * 72)
    _print("C2 wrong-branch reference patch -- record")
    _print(f"run: {run_dir}")
    _print("NOTE: this survey is only as current as the local pull. Refresh")
    _print("first, and re-survey after the training array drains:")
    _print("  " + _pull_refresh_command(run_dir))
    _print("=" * 72)

    try:
        survey_rows = survey_run(run_dir, specs=specs)
    ...
    if not survey_rows:
        ...
        return 0
    _print(format_survey_table(survey_rows, run_dir))
    _print()

    unknown = [r for r in survey_rows if r.state in ("unknown", "no-c2")]
    if unknown:
        for r in unknown:
            if r.state == "no-c2":
    ...
    patchable = [r for r in survey_rows if r.patchable]
    pending = [r for r in survey_rows if r.pending_fetch]
    n_wrong = sum(1 for r in survey_rows if r.state == "wrong")
    _print(f"survey: {len(survey_rows)} channels; {n_wrong} wrong, "
           f"{len(patchable)} patchable now, {len(pending)} pending fetch")

    if args.dry_run:
        _print("--dry-run: stopping after the survey")
        return 0
    if not patchable:
    ...
    try:
        return _run_patch(run_dir, args, survey_rows, patchable, pending)
...
def _gate_stamped_wrong(survey_rows) -> None:
    ...
    bad = [r for r in survey_rows if r.state == "wrong" and r.stamped]
    if bad:
        ...
        raise PatchRefused(
            ...
            "the channel from the cluster and re-survey before patching.")


def _gate_c_atom(survey_rows, plans) -> float:
    """The recorded C-atom E_pbe must agree across every surveyed channel
    (the reaction recompute REUSES those recorded energies)."""
    values = []
    plan_paths = {p.record.path for p in plans}
    plan_by_path = {p.record.path: p for p in plans}
    for r in survey_rows:
...
def _gate_reaction_consistency(plan: ChannelPlan) -> None:
    ...
    if not any(r.get("name") == REACTION_NAME for r in rows):
        raise PatchRefused(
            f"spec {plan.record.spec} {plan.record.channel}: "
...
def _gate_solver_describe(plan: ChannelPlan, sc) -> None:
    ...
    if recorded != rebuilt:
        ...
        raise PatchRefused(
            f"spec {plan.record.spec} {plan.record.channel}: the rebuilt "
...
def _gate_coldstart_verify(plan: ChannelPlan, fresh: dict, *, tol_e: float,
    ...
    for field, tol in checks:
        ...
        if not (_is_num(a) and _is_num(b)):
            raise PatchRefused(
                f"spec {plan.record.spec} {plan.record.channel}: cold-start "
        ...
        if d > tol:
            raise PatchRefused(
                f"spec {plan.record.spec} {plan.record.channel}: the "
...
def _gate_model_free_recompute(plan: ChannelPlan, fresh: dict,
    ...
    must reproduce the exact agreed value bit-for-bit -- a mismatch
    ...
    for k in SCF_DEPENDENT_MODEL_FREE:
        ...
        if not (lo - band <= float(f) <= hi + band):
            raise PatchRefused(
                f"spec {plan.record.spec} {plan.record.channel}: the local "
...
def _run_patch(run_dir, args, survey_rows, patchable, pending) -> int:
    t_start = time.time()

    # ---- preconditions (no reads beyond the survey, no writes) -----------
    refs_dir = _gate_bench_refs(run_dir, args.bench_refs_dir)
    _gate_stamped_wrong(survey_rows)
    ...
    for record in patchable:
        plan = _load_channel(record)
        if plan.old_c2 is None:
            raise PatchRefused(
                f"spec {record.spec} {record.channel}: no {SPECIES!r} row in "
                "per_molecule.json where one is expected.")
        _gate_reaction_consistency(plan)
        plans.append(plan)

    # The agreement and C-atom pools are ALWAYS run-wide: a --specs
    ...
    specs = _parse_specs(args.specs)
    full_rows = (survey_rows if specs is None
                 else survey_run(run_dir, specs=None))

    c_atom_e_pbe = _gate_c_atom(full_rows, plans)
    _print(f"C-atom gate: recorded E_pbe({C_ATOM}) = {c_atom_e_pbe!r} "
           f"agrees across all surveyed channels (tol {C_ATOM_TOL:g})")

    pool, n_agree = _collect_agreement(run_dir, full_rows)
    if pool is None:
        raise PatchRefused(
            ...
            "exact agreed value (grid quantities). Re-pull after more specs "
            "complete, or repair one spec on the cluster first.")
    _env = pool["envelope"].get("E_pbe")
    _print(f"model-free agreed values from {n_agree} clean channels: "
    ...
    for i, plan in enumerate(plans, start=1):
        t0 = time.time()
        record = plan.record
        cell = cells.get(record.spec)
        if cell is None:
            raise PatchRefused(
                f"spec {record.spec} is absent from manifest.json's specs "
                f"list ({run_dir / 'manifest.json'}); the run dir is "
                "inconsistent and no cell identity exists to rebuild the "
                "arch/solver from.")
        _print(f"[{i}/{n}] spec {record.spec} {record.channel}: "
               f"recomputing (arch={cell['arch']}) ...")
        sc = _solver_config_for_channel(cfg, cell, record.channel)
        _gate_solver_describe(plan, sc)
        model_path = (run_dir / "checkpoints" / f"spec_{record.spec:04d}"
                      / record.model_file)
        md = _recompute_reference(cfg, cell, sc, refs_dir)
        _gate_reference(md, f"spec {record.spec} {record.channel}")
        ...
        if plan.coldstart:
            ...
            coldstart_deltas[(record.spec, record.channel)] = deltas
        ...
        _print(f"[{i}/{n}] spec {record.spec} {record.channel}: planned "
    ...
    _print()
    _print(_push_sheet(run_dir, [p.record for p in plans]))
    return 0
