...
MODE --local (Stage 0, structural reproduction, no cluster needed)
    ...
    ``xcquinox/data/haunschild_g2/g2_97.traj`` and run the DECOMPOSITION
...
from xcquinox.pipeline.config import ARCHITECTURES, TrainingSpec
...
from xcquinox.pipeline import solver_manual as solver_manual_mod
from xcquinox.pipeline import oneshot as oneshot_mod
from xcquinox.pipeline import models as pipeline_models
...
from xcquinox.pipeline.defused_grad import (
    defused_value_and_grad, _energy_trajectory_jit, _energy_trajectory_vjp_jit)
from xcquinox.pipeline.energy_override import set_energy_override, reset_energy_override
from xcquinox.pipeline.oneshot import scf_loss_tail_weights
...
from xcquinox.pipeline.cluster.domain import bh76_meta_to_loss_dict
...
def apply_ablation(name):
    ...
    if name == "ec_tail":
        ...
        thr = pipeline_models._NN_TAIL_THRESHOLD
