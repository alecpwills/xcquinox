# hpcjobs -- running the `xcquinox.pipeline.cluster` HPC training harness

This directory is a **job-staging area** for the SLURM training harness
(`xcquinox/pipeline/cluster/`). It is deliberately kept *outside* the importable
`xcquinox/` package.
...
pre-process (`tools/generate_holdout_representative_subsets.py`, whose alpha_on ledger
at sizes 1 to 6 is tracked as `hpcjobs/ledgers/bh76w411_repr_alpha_on_r1-6.json`).
The harness **consumes** the existing ledger
...
- **Golden faithfulness test.** `test_cluster_spec_golden.py` skips until you
  capture the snapshot once: `python tools/capture_notebook_spec_snapshot.py`.
- **`prepare` subcommand.** `python -m xcquinox.pipeline.cluster prepare <grid>`
