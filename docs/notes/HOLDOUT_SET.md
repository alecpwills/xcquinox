...
from the canonical pool builder (`xcquinox.pipeline.full_benchmark_pools.load_full_held_out_pools`)
...
(the c2 reference-grid drift regenerated 2026-07-26, so c2
...
- BH76: GMTKN55-BH76 forward barrier heights (Goerigk, Hansen, Bauer, Ehrlich, Najibi, Grimme, PCCP 19 32184 (2017); data/gmtkn55/BH76/.res)
- W4-11: GMTKN55-W4-11 zero-point-exclusive nonrelativistic atomization energies (Karton, Daon, Martin, Chem. Phys. Lett. 510, 165 (2011); data/gmtkn55/W4-11/.res)

Reference densities for the density channels are CCSD (not CCSD(T)) at the run's own
basis/grid (`xcquinox.pipeline.benchmark_refs`); reaction reference energies are the pool
...
## 7. The diet150 pool and the overlap reports

The Diet GMTKN55 set at 150 reactions is a held-out pool of its own
(`xcquinox/pipeline/data/diet150_pool.json`, built by `xcquinox.pipeline.gmtkn55_sets` from
`data/dietgmtkn55-150/AllElements-150.yaml`, `SubsetGMTKN55_150.yaml` and the checkout, and
regenerated by `tools/rebuild_full_benchmark_pools.py`). The element list defines the
reactions -- species, stoichiometric counts, reference energies and the subset weights --
and the checkout supplies every species' charge, spin and, where its internal distances
agree with the list's positions, its geometry; where the checkout's geometry differs from
the list's (nine species of NBPRC and S66, by up to a few thousandths of an Angstrom), the
list's positions are used, since the list's reference energies belong to the list's
geometries, and the species record states the source and the deviation. Where the element
list's reference differs from the checkout's `.res` line (four UPU23 reactions, and the
diet's BH76RC entry, which is a BH76 barrier) the element list is the definition. Nothing
is excluded: every reaction of the 49 subsets is in the pool with its weight, and a run
names the pool with `inputs.held_out_pools`.
...
beside it, and the `test_set_held_out_combined` row stays the BH76 + W4-11 pair. Species of the diet set that
are BH76 or W4-11 species carry those pools' names (one energy, one reference file per name);
every other species is named with its subset's tag, and a W4-11 name that BH76 carries with
another geometry is named `w411_<system>`. The two tracked pools themselves share fourteen
names with different geometries, of which the union keeps BH76's, as every evaluation of the
pair has done; the evaluation log names them.
