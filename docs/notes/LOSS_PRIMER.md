...
five-channel loss class (`xcquinox/pipeline/losses.py:1063`):
...
(`xcquinox/pipeline/train.py:1737-1743`):
...
these defaults) and applied in `xcquinox/pipeline/defused_grad.py:156`:
...
1. The YAML sets no `update_scheme`, so the cluster default applies
   (`xcquinox/pipeline/cluster/grid_config.py:148-153`):
...
The loss is *not* GradNorm-balanced at run time: GradNorm(alpha=1.5) is dormant and the
effective weights are {AE:1, BH76:1, IP13:1, vxc:1, rho:20}. A description of GradNorm as
discovering task weights adaptively applies to the batched path, not the production path.

### 1.2 What each channel computes

All residuals are Hartree-derived; references arrive in kcal/mol and are converted
once (`KCAL_PER_HA = 627.5094740631`, `xcquinox/pipeline/cluster/domain.py:27`;
...
(`xcquinox/pipeline/config.py:740`) and no harness path overrides it, so the
...
- **loss_AE** (`losses.py:1326`, via `_ae_losses` at `:246`) = AE fitting term +
  ...
  - THE FITTING TERM'S REALIZED FORM DIFFERS BY RUN GENERATION. The source YAMLs
    ...
    (`xcquinox/pipeline/training_points.py:133`), with the names force-added to
...
(`xcquinox/pipeline/oneshot.py:632`): keep the last `min(tail, N)` steps
...
(`decaying_linear`, `alpha = 0.3**step + 0.3`, `xcquinox/pipeline/solver.py:269`,
...
term; this is the dominant effect (the Letter
...
`xcquinox/pipeline/pretrain.py:1299`). The objective is an integration-weighted MSE
...
| Degenerate-eigenvalue guard | Random half-normal V_xc noise, std 1e-8, symmetrized (SI Sec. IV) | Deterministic SYM_BREAK_SHIFT 1e-6 diagonal |
...
| Self-consistency per point class | Nine of the AE molecules (NO2, NH, O3, N2O, CH3, CH2, H2O, NH3, O2) and every BH76 reaction entry staged non-self-consistently ('sc': False in the reference trajectory: one pass at the converged density, no per-molecule loss, the group's energy loss x nonsc_weight 0.5); the other AE molecules, the atoms and the IP entries self-consistent | Off by default: every point class self-consistent under one solver_config. `hyperparams.respect_sc_flag` with `nonsc_weight` reproduces the split from the pool's flags (`dfs_pool.DFS_NONSC_AE_HILLS`, the reaction dicts), recorded on every spec as `nonsc_points` |
