...
3x3 grids saved in `figures/` beside this note (the demo scripts under `tools/analysis/` regenerate them):
...
(`xcquinox/pipeline/oneshot.py`, `xcquinox/pipeline/solver.py`,
`xcquinox/pipeline/solver_manual.py`) -- not just the docstring.
...
| **+LO** | `lieb_oxford` on the exchange net | Enforces the **Lieb-Oxford bound** -- an exact lower bound on the exchange-correlation energy (E_xc ≥ −C · ∫ρ^{4/3}). Implemented as a smooth output squash that prevents the network from emitting unphysically negative enhancement factors. |
...
| **`xcquinox.pipeline.solver`** | Where `SolverConfig`, `SolverMode`, `FeaturePolicy`, and `SolverBackend` are defined. |
| **`xcquinox.pipeline.pretrain`** | Defines `PretrainSpec`, `loss_weighting`, integration weights. |
