# `xcquinox.pipeline` training pipeline: code-flow diagrams

Flowcharts of the machine-learned-XC training workflow: the SLURM cluster
harness (`xcquinox/pipeline/cluster/`) that orchestrates the job graph, and the core compute
(`xcquinox/pipeline/`) that runs inside each job, model build, the training-loop family, the
...
JupyterLab) and a higher-resolution Graphviz-rendered image under `diagrams/`.
Node labels are `file.py::function`, so the diagrams double as a
code index.
...
`python -m xcquinox.pipeline.cluster submit config.yaml` -> `__main__.py::cmd_submit` ->
...
```mermaid
flowchart TB
  cli(["$ python -m xcquinox.pipeline.cluster submit config.yaml"]) --> submit["__main__::cmd_submit<br/>submit.py::submit_jobs<br/>create run_&lt;UTC&gt;Z/ dir"]
...
(`filter_value_and_grad -> optimizer.update -> apply_updates`), guarded by the fail-loud
`_abort_if_nonfinite`, which raises immediately on a non-finite loss/gradient, naming the
loop/step/group/channel. Final state is written by `_save_artifacts`.

```mermaid
flowchart TB
  task(["_train_task::main (array task i)<br/>load specs/spec_i.spec"]) --> worker["worker: xcquinox.pipeline._train_one_spec<br/>-> run_training(spec)"]
...
The polarized-correlation NaN guard lives in the `compute_vxc_nn` /
...
frozen, to the per-point Fx/Fc enhancement targets under `|rho*eps_LDA|` integration weights,
...
- The harness stages (diagram 1) invoke the per-stage Python entrypoints under
  `xcquinox/pipeline/cluster/` (`_datagen`, `_pretrain`, `_preflight`, `_train_task`,
  `_eval_one_spec`).
