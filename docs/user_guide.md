# xcquinox.pipeline user guide: training an ML exchange-correlation functional

A practical guide to using the `xcquinox.pipeline` toolkit. It covers (1) training and evaluating a
...
[architecture/pipeline_training_flow.md](architecture/pipeline_training_flow.md).
...
4. Build a `TrainingSpec`. Choose the architecture, the loss, and how long to train:
   ...
   ```python
   from xcquinox.pipeline.config import TrainingSpec, get_architecture
...
6. Evaluate. Re-load `model.eqx` into a `TestSpec` and score it with named metrics
   ...
   ```python
   from xcquinox.pipeline.config import TestSpec
...
four training notebooks under `notebooks/` (`gga_training_pipeline`, `gga_training_scf_solvers`,
`gga_training_anchor_transfer` and `gga_training_dfs_subsets`, each generated by its builder, see
`notebooks/README.md`) work through progressively richer versions of the same flow.
...
[hpcjobs/SEAWULF_RUNBOOK.md](https://github.com/alecpwills/xcquinox/blob/main/hpcjobs/SEAWULF_RUNBOOK.md),
which stays beside the job scripts it documents. This section explains what the commands
...
All commands are `python -m xcquinox.pipeline.cluster <subcommand> ...`.

```bash
# Preview the job graph WITHOUT queuing anything (submit is dry-run by default):
python -m xcquinox.pipeline.cluster submit hpcjobs/configs/bh76w411_repr.svp_grid2.yaml \
...
python -m xcquinox.pipeline.cluster pull auto
# Single runs still pull by stamp or 'latest' per category:
python -m xcquinox.pipeline.cluster pull latest --category bh76w411_repr/svp_grid2/runs
...
python -m xcquinox.pipeline.cluster results <local_run_dir> --csv out.csv --plot mae_vs_subset.png
```
...
- Cluster setup specifics (SeaWulf env, partitions, exact paths):
  [hpcjobs/SEAWULF_RUNBOOK.md](https://github.com/alecpwills/xcquinox/blob/main/hpcjobs/SEAWULF_RUNBOOK.md).
- Worked notebooks: the four `notebooks/gga_training_*.ipynb`, progressively richer end-to-end
  runs, each generated by its builder (`notebooks/README.md`).
